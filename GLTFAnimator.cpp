...
void GLTFAnimator::UpdateAnimations(float deltaTime, Model* sceneModels, int maxModels)
{
    try
    {
        ...
        // Helper: Determine if a model instance belongs to the given root parent hierarchy.
        // This uses the scene model instance indices as IDs, and walks iParentModelID.
        auto IsInHierarchy = [&](int modelIndex, int rootParentID) -> bool
        {
            // Safety checks
            if (modelIndex < 0 || modelIndex >= maxModels)
                return false;

            // Root must be valid
            if (rootParentID < 0 || rootParentID >= maxModels)
                return false;

            // Walk up the parent chain until we either reach the root or hit a sentinel.
            int current = modelIndex;
            while (current >= 0 && current < maxModels)
            {
                if (current == rootParentID)
                    return true;
        ...
        // Helper: Reset animated local TRS values to the captured base local TRS.
        auto ResetLocalTRSToBase = [&](int rootParentID)
        {
            for (int i = 0; i < maxModels; ++i)
            {
                ...
                if (!IsInHierarchy(i, rootParentID))
                    continue;

                if (!sceneModels[i].m_modelInfo.bHasBaseLocalTRS)
                    continue;

                sceneModels[i].m_modelInfo.animLocalTranslation = sceneModels[i].m_modelInfo.baseLocalTranslation;
                sceneModels[i].m_modelInfo.animLocalRotationQuat = sceneModels[i].m_modelInfo.baseLocalRotationQuat;
                sceneModels[i].m_modelInfo.animLocalScale = sceneModels[i].m_modelInfo.baseLocalScale;
            }
        };

        // Helper: Recompose world matrices from local TRS using parent-child hierarchy.
        auto RecomposeWorldFromLocalTRS = [&](int rootParentID)
        {
            // NOTE: This assumes parents are created before children, which is true for our recursive scene parsing.
            for (int i = 0; i < maxModels; ++i)
            {
                if (!sceneModels[i].m_isLoaded)
                    continue;
                ...
                // Build the LOCAL matrix from animated TRS.
                const XMFLOAT3& t = sceneModels[i].m_modelInfo.animLocalTranslation;
                const XMFLOAT4& q = sceneModels[i].m_modelInfo.animLocalRotationQuat;
                const XMFLOAT3& s = sceneModels[i].m_modelInfo.animLocalScale;
                ...
                XMMATRIX localM = XMMatrixScalingFromVector(sV) * XMMatrixRotationQuaternion(qV) * XMMatrixTranslationFromVector(tV);

                // Compose WORLD = PARENT_WORLD * LOCAL (or LOCAL if no parent).
                int parentID = sceneModels[i].m_modelInfo.iParentModelID;
                ...
                // Update convenience world position (translation) from worldMatrix.
                XMFLOAT4X4 w;
        ...
        for (auto& instance : m_animationInstances)
        {
            ...
            // Reset all local TRS values back to base before applying channels.
            // This prevents drift and ensures animations evaluate as absolute LOCAL transforms.
            ResetLocalTRSToBase(instance.parentModelID);
            ...
            // After all channels have been applied (LOCAL TRS), recompose WORLD matrices through the hierarchy.
            RecomposeWorldFromLocalTRS(instance.parentModelID);
...
void GLTFAnimator::ApplyAnimationToNode(const AnimationChannel& channel, const std::vector<float>& values, Model* sceneModels, int maxModels, int parentModelID)
{
    try
    {
        ...
        for (int i = 0; i < maxModels; ++i)
        {
            ...
            // Only apply to the exact GLTF node targeted by this channel.
            // Without this mapping, animations will be incorrectly applied across the entire hierarchy.
            if (sceneModels[i].m_modelInfo.gltfNodeIndex < 0)
                continue;

            if (sceneModels[i].m_modelInfo.gltfNodeIndex != channel.targetNodeIndex)
                continue;

            // Apply animation based on target path.
            // IMPORTANT: GLTF animation channels provide LOCAL TRS values.
            // We write these into animLocal* fields and let UpdateAnimations() recompose world matrices.
            switch (channel.targetPath)
            {
                ...
                case AnimationTargetPath::ROTATION:
                {
                    if (values.size() >= 4)
                    {
                        ...
                        // Store the quaternion as the LOCAL animated rotation.
                        sceneModels[i].m_modelInfo.animLocalRotationQuat = animRotation;
                        ...
                        // Store the converted Euler angles for legacy debug/inspection ONLY.
                        // The renderer uses worldMatrix; recomposition is performed in UpdateAnimations().
                ...
                case AnimationTargetPath::SCALE:
                {
                    if (values.size() >= 3)
                    {
                        // Update the model's LOCAL scale with animated scale values
                        sceneModels[i].m_modelInfo.animLocalScale.x = values[0];
                        sceneModels[i].m_modelInfo.animLocalScale.y = values[1];
                        sceneModels[i].m_modelInfo.animLocalScale.z = values[2];
            ...
            // NOTE: worldMatrix recomposition is handled in UpdateAnimations() after all channels are applied.
        }
    }
    catch (const std::exception& ex)
    {
