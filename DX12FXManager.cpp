...
void FXManager::AddEffect(const FXItem& fxItem) {
    ...
    // If called from inside Render() (e.g. from a FadeOutThenCallback callback), defer
    // the push_back so that the reallocation never occurs while 'effects' is being iterated.
    // The pending list is flushed at the end of Render(), after all iteration is complete.
...
void FXManager::Render(bool backgroundOnly) {
    ...
    try {
        ...
        // Update background effects only in the background pass.
        // The foreground pass (backgroundOnly=false) runs microseconds later so
        // dt≈0 — running the update loop again is wasted work and accumulates
        // redundant RemoveCompletedEffects / ThreadLockHelper overhead.
        if (backgroundOnly) {
            for (auto& fx : effects) {
        ...
        // backgroundOnly=true  → pass-1 (before 3D): Starfield, WarpDotTunnel
        // backgroundOnly=false → pass-2 (after  3D): ColorFader overlay
        for (auto& fx : effects) {
        ...
        // Only clean up in the foreground pass — starfield/tunnel run indefinitely and
        // color faders only advance in backgroundOnly=false, so there is nothing to
        // remove after the background pass.  Skipping here saves one ThreadLockHelper
        // acquisition per frame.
        if (!backgroundOnly) RemoveCompletedEffects();
        RestoreRenderState();

        // Flush any effects that were queued while bIsRendering was true
        // (e.g. FadeToImage called from a FadeOutThenCallback).  This is done
        // AFTER all iteration so that push_back never invalidates live iterators.
        {
            std::lock_guard<std::mutex> lock(m_effectsMutex);
...
void FXManager::CreateStarfield(int numStars, float circularRadius, float resetDepthPos, XMFLOAT3 startPos, bool reverse)
{
    ...
    for (int i = 0; i < numStars; ++i) {
        ...
        p.speed    = 120.0f + static_cast<float>(rand()) / RAND_MAX * 240.0f; // [120, 360] — 2× previous range for a faster SCENE_GAMETITLE starfield
        ...
        for (auto& p : fx.particles) {
            ...
            if (!reverse) {
                ...
                if (zPos <= 5.0f) {
                    ...
                    p.speed  = 120.0f + static_cast<float>(rand()) / RAND_MAX * 240.0f; // [120, 360] — keep respawn speed in sync with CreateStarfield
...
void FXManager::RenderStarfield(FXItem& fxItem) {
    ...
    for (auto& p : fxItem.particles) {
        if (p.completed) continue;
        // Skip nearly-invisible stars to reduce D2D FillRectangle calls in the hot path
...
{
    if (!m_hasZoomConfig) {
        debug.logLevelMessage(LogLevel::LOG_WARNING, L"[DX12FXManager] StartZoom called before ZoomInitialise -- ignored.");
        return;
    }
    StopZooming();
...
void FXManager::StartFireworks(float freqRate)
{
    ...
    fw.baseY       = static_cast<float>(renderer->iOrigHeight);         // Rockets launch from the bottom of screen
    fw.rockets.clear();

    fireworksID = newFX.fxID;
    effects.push_back(newFX);

    debug.logLevelMessage(LogLevel::LOG_INFO, L"[DX12FXManager] Fireworks started: FreqRate=" +
...
void FXManager::StopFireworks()
{
    ...
    debug.logLevelMessage(LogLevel::LOG_INFO, L"[DX12FXManager] Fireworks stopped.");
    fireworksID = 0;
}

// ============================================================
// UpdateFireworks — per-frame simulation tick: advances timers, launches new rockets, moves rockets
//   and particles, computes screen positions, then prunes completed rockets. Does NOT draw anything.
...
// ============================================================
// DrawFireworksPixels — internal helper: pixel-draws all rockets and particles in one FXItem.
...
void FXManager::DrawFireworksPixels(FXItem& fx)
{
    ...
    for (auto& r : fw.rockets)
    {
        ...
        else
        {
            ...
            // All particles in a burst share the same RGB; alpha varies per step.
            for (auto& p : r.expParticles)
            {
                if (p.completed) continue;
                ...
                for (int t = 0; t < TRAIL_STEPS; ++t)
                {
                    ...
                    // Quadratic alpha fade with additional trail-step falloff; skip near-invisible draws
...
// ============================================================================================================
// ImageFadeStrobe — DX12 implementation (identical logic to DX11)
// ============================================================================================================
...
void FXManager::StartImageFadeStrobe(BlitObj2DIndexType type, float fadeOutPercentage, float fadeOverTime)
{
    ...
    if (strobeCount >= MAX_STROBE_INSTANCES) {
        debug.logLevelMessage(LogLevel::LOG_WARNING, L"[FXManager DX12] ImageFadeStrobe: max instances reached");
        return;
    }
    ...
    debug.logLevelMessage(LogLevel::LOG_INFO, L"[FXManager DX12] ImageFadeStrobe started: image=" +
...
void FXManager::StopImageFadeStrobe(BlitObj2DIndexType type)
{
    ...
    debug.logLevelMessage(LogLevel::LOG_INFO, L"[FXManager DX12] ImageFadeStrobe stop requested: image=" +
