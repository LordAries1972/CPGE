...
void FXManager::UpdateFireworks(FXItem& fx)
{
    ...
    if (fw.launchTimer >= fw.freqRate && static_cast<int>(fw.rockets.size()) < 10)
    {
        ...
        // Random explosion radius up to 200 px; random particle count 1-15
...
void FXManager::DrawFireworksPixels(FXItem& fx)
{
    ...
    for (auto& r : fw.rockets)
    {
        ...
        else
        {
            ...
            // All particles in a burst share the same RGB; only alpha varies per particle.
            for (auto& p : r.expParticles)
            {
                if (p.completed) continue;
                ...
                // Skip nearly-invisible particles to reduce D2D FillRectangle calls
