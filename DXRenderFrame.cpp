...
void DX11Renderer::RenderFrame()
{
    ...
    try
    {
        ...
        RECT rc;                                                        // Client rectangle for viewport calculation
            ...
            if (m_d3dDevice)
            {
                ...
                if (FAILED(deviceStatus) && (!threadManager.threadVars.bIsResizing.load()))
                {
                    ...
                    if (!sysUtils.IsWindowMinimized()) 
                    {
                        ...
                        try {
                            // Attempt device reset through resize
                        ...
                        // Exit after device reset attempt
                        return;                                             
                    }
                    else
                    {
            ...
            // Update camera animation and timing
            myCamera.UpdateJumpAnimation();

            // Get current high-resolution time
            auto now = std::chrono::steady_clock::now();                                   
            // Calculate raw delta time in seconds
            float rawDeltaTime = std::chrono::duration<float>(now - lastFrameTime).count(); 
            
            // Target 60 FPS for smoothing
            float smoothedDeltaTime = deltaTimeSmoothing.ProcessDelta(rawDeltaTime, 60.0f); 
            // Use smoothed delta time for animations
            ...
            lastFrameTime = now;
                ...
                if (frameCounter >= 60)
                {
                    debug.logDebugMessage(LogLevel::LOG_DEBUG, L"[RENDERFRAME] Delta time - Raw: %.6f, Smoothed: %.6f", rawDeltaTime, deltaTime);
                    frameCounter = 0;
                }
            #endif
            ...
            // 2D Rendering with enhanced thread safety
                ...
                if (d2dRenderLock.IsLocked())
                {
                        ...
                        (bIsInitialized.load()))
                    {
                        ...
                        switch (scene.stSceneType)
                        {
                           ...
                           {
                               RenderIntroMovie(); // Render intro movie scene
                               break;
                           }
                           ...
                           case SceneType::SCENE_GAMETITLE:
                           {
                               ...
                               // TSOO image blit runs in this 2D overlay pass (after RenderGamePlay)
                               // so it composites ON TOP of all 3D model rendering
                               if (threadManager.threadVars.bLoaderTaskFinished.load() &&
                                   m_d2dTextures[int(BlitObj2DIndexType::IMG_TSOO)])
                               {
                   ...
                   if (USE_FPS_DISPLAY && config.myConfig.showDebugInfo)
                   {
                       ...
                       DrawMyText(fpsText, Vector2(5.0f, 5.0f), MyColor(255, 255, 255, 255), dbgFontSize);
                   }
                   ...
                   if (!threadManager.threadVars.bLoaderTaskFinished.load())
                   {
                       ...
                       if (delay > 3)                                         // Update animation every 3 frames
                       {
                   ...
                   // Apply 2D post-processing effects
                   try {
           ...
           try {
               ...
               // Capture back-buffer BEFORE Present so the fully-composed frame is recorded
               ...
               if (m_swapChain) {
                   // Capture the start time of the present for optional software frame limiting.
                   std::chrono::steady_clock::time_point presentStart = std::chrono::steady_clock::now();

                   // Present with VSync when enabled (sync interval 1).
                   // Present with sync interval 0 when disabled (unlocked).
                   UINT syncInterval = config.myConfig.enableVSync ? 1 : 0;
                   HRESULT presentResult = m_swapChain->Present(syncInterval, 0);
                   ...
                   // This also stabilizes timing on systems where Present(0,0) runs at uncapped speeds.
                   if (!config.myConfig.enableVSync) {
                       // Target ~60 FPS (16 ms) as a safe default cap when unlocked.
...
/* ---------------------------------------------------------------------
   This function is for the SceneType::SCENE_GAMEPLAY rendering logic
   --------------------------------------------------------------------- */
inline void DX11Renderer::RenderGamePlay(float deltaTime)
{
    ...
    if (m_d3dContext && m_cameraConstantBuffer)
    {
        ...
        // Update camera constant buffer for 3D rendering
        ...
        if (SUCCEEDED(hr)) {
            // Copy camera data to GPU — asm MemoryCopy: REP MOVSQ throughput
            MemoryCopy(&cb, mappedResource.pData, sizeof(ConstantBuffer));
            // Unmap the buffer
        ...
        // the light buffer — mirrors the Vulkan and OpenGL render paths.
        ...
        // drawn with stale/zero light data on every frame.
        {
            ...
            // Bulk copy all lights at once — single REP MOVSQ pass instead of per-element memcpy loop
            MemoryCopy(globalLights.data(), glb.lights, static_cast<size_t>(glb.numLights) * sizeof(LightStruct));
            ...
            if (SUCCEEDED(hrLight))
            {
                ...
                // asm MemoryZero: REP STOSQ — clears at hardware bandwidth
                static const UINT kGlobalLightCBMinBytes = 1728;
                MemoryZero(mapped.pData, kGlobalLightCBMinBytes);
                // asm MemoryCopy: REP MOVSQ — uploads struct at hardware bandwidth
                MemoryCopy(&glb, mapped.pData, sizeof(GlobalLightBuffer));
        ...
        if (threadManager.threadVars.bLoaderTaskFinished.load())
        {
            ...
            {
                if (scene.scene_models[i].m_isLoaded && !scene.scene_models[i].m_modelInfo.bIsTransformProxy)   // Only render loaded mesh models
                {
...
/* ---------------------------------------------------------------------
   This function is for the SceneType::SCENE_INTRO_MOVIE rendering logic
   --------------------------------------------------------------------- */
inline void DX11Renderer::RenderIntroMovie()
{
    #if defined(_DEBUG_RENDERER_) && defined(_DEBUG)
    ...
    if (moviePlayer.IsPlaying()) {
        ...
        // UpdateFrame() is called before D2D BeginDraw (in RenderFrame) to keep all
        // D3D11 context->Map() calls outside BeginDraw (SEH exception 0x87D guard).
...
void DX11Renderer::RenderBackgroundImage()
{
    ...
    switch (scene.stSceneType)
    {
        ...
        case SceneType::SCENE_EXPERIMENT:
        {
            if (threadManager.threadVars.bLoaderTaskFinished.load())
            {
                // Black background — render tunnel dots on top
                if (fxManager.tunnelID > 0)
                    fxManager.RenderFX(fxManager.tunnelID, m_d3dContext.Get(), myCamera.GetViewMatrix());
            }
            break;
