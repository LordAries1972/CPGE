...
void main() {
    ...
    vec3  col   = (uAmbient + diff * uLightColor) * tex.rgb;
                                   ...
                                   const MyColor& tint, bool wrap)
{
    ...
    // Build an orthographic projection matching screen pixels (top-left origin)
    ...
    // Column-major ortho: [-1..+1] in X, [+1..-1] in Y (top-left = NDC top-left)
    float ortho[16] = {
    ...
    if (textureID && (srcW > 0 && srcH > 0)) {
        ...
        u0 = static_cast<float>(srcX) / tw;
        v0 = static_cast<float>(srcY) / th;
        u1 = static_cast<float>(srcX + srcW) / tw;
        v1 = static_cast<float>(srcY + srcH) / th;
    }
...
// ─────────────────────────────────────────────────────────────────────────────
// Texture loading (GDI+ on Windows)
// ─────────────────────────────────────────────────────────────────────────────
bool OpenGLRenderer::LoadTexture(int textureId, const std::wstring& filename, bool is2D)
{
    ...
    if (bmp.GetLastStatus() != Gdiplus::Ok) {
        debug.logLevelMessage(LogLevel::LOG_ERROR,
            L"[OpenGLRenderer] LoadTexture: GDI+ failed to open: " + filename);
        return false;
    }
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, W, H, 0, GL_BGRA_EXT, GL_UNSIGNED_BYTE, bd.Scan0);
