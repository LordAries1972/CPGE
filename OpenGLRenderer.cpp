                                   ...
                                   const MyColor& tint, bool wrap)
{
    ...
    // Y is negated so screen Y=0 maps to NDC Y=+1 (top).
    ...
    // Column-major layout for glUniformMatrix4fv(GL_FALSE):
    // col0=(2/W,0,0,0), col1=(0,-2/H,0,0), col2=(0,0,-1,0), col3=(-1,1,0,1)
    float ortho[16] = {
    ...
    GLint locOrtho  = glGetUniformLocation(m_2dShaderProgram.programID, "uOrtho");
    GLint locColor  = glGetUniformLocation(m_2dShaderProgram.programID, "uColor");
    GLint locUseTex = glGetUniformLocation(m_2dShaderProgram.programID, "uUseTex");
    GLint locTex    = glGetUniformLocation(m_2dShaderProgram.programID, "uTex");

    if (locOrtho >= 0) glUniformMatrix4fv(locOrtho, 1, GL_FALSE, ortho);
    if (locColor >= 0) glUniform4f(locColor,
    ...
    const MyColor& color, const float fontSize, const std::wstring& fontName)
{
    if (text.empty()) return;
    int tw = 0, th = 0;
    GLuint texID = RenderTextToTexture(text, fontName, fontSize, color, tw, th);
    if (!texID) return;
    Render2DQuad(texID,
        static_cast<int>(position.x), static_cast<int>(position.y),
        tw, th, 0, 0, tw, th, MyColor(255, 255, 255, 255), false);
...
{
    return fontSize * 1.2f; // single-line approximation
}
