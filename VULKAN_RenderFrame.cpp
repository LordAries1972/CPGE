...
inline void VulkanRenderer::RenderGamePlay(float deltaTime)
{
    ...
    for (int i = 0; i < MAX_MODELS; ++i)
    {
        ...
        // TODO: Vulkan GPU vertex/index buffers not yet provisioned in ModelInfo.
        // DrawVulkan will be added once VkBuffer/VkDescriptorSet fields are added
        // to ModelInfo under __USE_VULKAN__ alongside the DX11 GPU buffer fields.
        (void)ubo;
    }
}
...
{
    // Movie frames are delivered as VkImage / CPU pixel data depending on MoviePlayer implementation.
    // The movie player writes each decoded frame into the overlay texture (Windows path: via D2D + WIC).
    // For now: request the current frame from MoviePlayer and flag overlay dirty.
#if defined(PLATFORM_WINDOWS)
    if (m_d2dRenderTarget && moviePlayer.IsPlaying())
    {
        m_overlayDirty = true;
    }
#elif defined(PLATFORM_LINUX) || defined(PLATFORM_ANDROID)
    // Linux/Android: MoviePlayer supplies raw RGBA frames; upload directly to overlay staging buffer
...
void VulkanRenderer::RenderFrame()
{
    ...
    try
    {
               ...
               (!threadManager.threadVars.bIsShuttingDown.load()))
        {
            ...
            if (m_d2dRenderTarget) {
                switch (scene.stSceneType) {
                    case SceneType::SCENE_INTRO:
                        if (m_textures2D[int(BlitObj2DIndexType::IMG_SPLASH1)].isValid)
                ...
                if (!threadManager.threadVars.bLoaderTaskFinished.load()) {
                    ...
                    if (m_textures2D[int(BlitObj2DIndexType::BG_LOADER_CIRCLE)].isValid) {
                ...
                // Cursor
                if (m_textures2D[int(BlitObj2DIndexType::BLIT_ALWAYS_CURSOR)].isValid)
            ...
            VkSemaphore          signalSems[] = { fd.renderFinished };
            ...
            VkResult presentResult = vkQueuePresentKHR(m_presentQueue, &present);
