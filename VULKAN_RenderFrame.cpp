...
inline void VulkanRenderer::RenderGamePlay(float deltaTime)
{
    ...
    // Animate lights (pulse / flicker / strobe) each frame before pushing the
    // light constant — mirrors the DX11 and OpenGL render paths.
    ...
    for (int i = 0; i < MAX_SCENE_MODELS; ++i)
    {
        ...
        if (m.m_modelInfo.uniformBufferMapped)
        {
                ...
                MemoryCopy(&ubo, m.m_modelInfo.uniformBufferMapped, sizeof(ubo));
            #else
...
void VulkanRenderer::RenderFrame()
{
    ...
    try
    {
               ...
               (!threadManager.threadVars.bIsShuttingDown.load()))
        {
            ...
            // UpdateAnimations is called unconditionally so GLTF hierarchy world matrices are
            // always valid (static base-pose and animated clips alike).
            switch (scene.stSceneType) {
                case SceneType::SCENE_GAMETITLE:
                    if (threadManager.threadVars.bLoaderTaskFinished.load())
