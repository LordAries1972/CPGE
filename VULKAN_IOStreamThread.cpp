...
/* ----------------------------------------------------------------
This is the placement code for the VulkanRenderer Loader Thread.
...
void VulkanRenderer::LoaderTaskThread()
{
           ...
           (status != ThreadStatus::Stopped))
    {
        ...
        switch (scene.stSceneType)
        {
            ...
            {
                // Helper that posts a loading stage message to the D2D overlay while the
                // loader is still running.  Mirrors DX11IOStreamThread::showStage().
                ...
                // Signal to the render frame that loading is in progress so it shows
                // the loading screen instead of the game intro background immediately.
                // This must be set BEFORE any work begins, not just b2DTexturesLoaded.
                threadManager.threadVars.bLoaderTaskFinished.store(false);
                threadManager.threadVars.b2DTexturesLoaded.store(false);
