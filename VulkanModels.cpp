...
namespace VulkanModelUtils
{
                                    ...
                                    std::mutex* queueMutex)
    {
        ...
        (void)path; (void)outImage; (void)outMemory; (void)queueMutex;
        ...
        return LoadTextureFromMemory(device, physDevice, cmdPool, queue,
                                     buf.data(), buf.size(), outImage, outMemory, queueMutex);
#endif
    }
...
#if defined(STBI_NOT_AVAILABLE)
        (void)device; (void)physDevice; (void)cmdPool; (void)queue;
        (void)data; (void)size; (void)outImage; (void)outMemory; (void)queueMutex;
        return VK_NULL_HANDLE;
#else
        int w = 0, h = 0, ch = 0;
    ...
    VkImageView CreateSolidColourTexture(VkDevice device, VkPhysicalDevice physDevice,
                                          ...
                                          uint32_t width, uint32_t height)
    {
        // Build RGBA pixel buffer directly — do NOT pass through stbi (needs file headers).
        const uint8_t r = static_cast<uint8_t>(std::clamp(colour.x, 0.0f, 1.0f) * 255.0f);
        const uint8_t g = static_cast<uint8_t>(std::clamp(colour.y, 0.0f, 1.0f) * 255.0f);
        const uint8_t b = static_cast<uint8_t>(std::clamp(colour.z, 0.0f, 1.0f) * 255.0f);
        const uint8_t a = static_cast<uint8_t>(std::clamp(colour.w, 0.0f, 1.0f) * 255.0f);
        ...
        return UploadPixelsToImage(device, physDevice, cmdPool, queue,
                                   pixels.data(), width, height, outImage, outMemory, queueMutex);
    }
}
