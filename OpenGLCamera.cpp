...
Camera::Camera() {
    ...
    UpdateViewMatrix();
        ...
        debug.logLevelMessage(LogLevel::LOG_INFO, L"OpenGL Camera created successfully with enhanced jump animation, history support, and continuous rotation");
    #endif
}
...
Camera::~Camera() {
    ...
    #if defined(_DEBUG_CAMERA_)
        debug.logLevelMessage(LogLevel::LOG_INFO, L"OpenGL Camera destroyed with complete jump animation, history, and rotation cleanup!");
    #endif
}
...
void Camera::RestoreCameraStateAfterResize()
{
    ...
    try {
        ...
        try {
            ...
            projectionMatrix = glm::perspective(fovRadians, aspectRatio, nearPlane, farPlane);
        }
        catch (...) {
...
void Camera::JumpToWithYawPitch(float new_x, float new_y, float new_z, float newYaw, float newPitch, int speed, bool FocusOnTarget)
{
    try {
        ...
        float clampedPitch     = std::clamp(newPitch, minPitch, maxPitch);
...
void Camera::RotateX(float degrees, int speed, bool FocusOnTarget)
{
    ...
    glm::mat4 rotMat = glm::rotate(glm::mat4(1.0f), angleRad, glm::vec3(1.0f, 0.0f, 0.0f));
    glm::vec3 rotatedVec = glm::vec3(rotMat * glm::vec4(vectorToRotate, 0.0f));
    rotatedVec = glm::normalize(rotatedVec) * rotDist;

    if (FocusOnTarget) {
        newPosition = pivotPoint + rotatedVec;
    } else {
        target = pivotPoint + rotatedVec;
    }
...
void Camera::RotateY(float degrees, int speed, bool FocusOnTarget)
{
    ...
    float angleRad  = glm::radians(degrees);
    glm::mat4 rotMat = glm::rotate(glm::mat4(1.0f), angleRad, glm::vec3(0.0f, 1.0f, 0.0f));
    glm::vec3 rotatedVec = glm::vec3(rotMat * glm::vec4(vectorToRotate, 0.0f));
    rotatedVec = glm::normalize(rotatedVec) * rotDist;
...
void Camera::RotateZ(float degrees, int speed, bool FocusOnTarget)
{
    ...
    float angleRad  = glm::radians(degrees);
    glm::mat4 rotMat = glm::rotate(glm::mat4(1.0f), angleRad, glm::vec3(0.0f, 0.0f, 1.0f));
    glm::vec3 rotatedVec = glm::vec3(rotMat * glm::vec4(vectorToRotate, 0.0f));
    rotatedVec = glm::normalize(rotatedVec) * rotDist;
...
void Camera::RotateXYZ(float xDegrees, float yDegrees, float zDegrees, int speed, bool FocusOnTarget)
{
    ...
    float radX = glm::radians(xDegrees);
    float radY = glm::radians(yDegrees);
    float radZ = glm::radians(zDegrees);

    // GLM is column-major (M*v), so to apply X then Y then Z we reverse multiply order
    ...
    glm::mat4 combined = rotMatZ * rotMatY * rotMatX;

    glm::vec3 rotatedVec = glm::vec3(combined * glm::vec4(vectorToRotate, 0.0f));
    rotatedVec = glm::normalize(rotatedVec) * rotDist;
...
glm::vec3 Camera::CalculateRotatedPosition(const glm::vec3& currentPos, const glm::vec3& pivot, float angleX, float angleY, float angleZ) const
{
    ...
    glm::vec3 rotated = glm::vec3(combined * glm::vec4(relPos, 0.0f));
...
std::vector<glm::vec3> Camera::CalculateSmoothTravelPath(const glm::vec3& start, const glm::vec3& end, int pathPoints) const
{
    ...
    for (int i = 0; i < pathPoints; ++i)
    {
        ...
        glm::vec3 point;
        point.x = invT3 * start.x + 3.0f * invT2 * smoothT * cp1.x + 3.0f * invT * t2 * cp2.x + t3 * end.x;
        point.y = invT3 * start.y + 3.0f * invT2 * smoothT * cp1.y + 3.0f * invT * t2 * cp2.y + t3 * end.y;
        point.z = invT3 * start.z + 3.0f * invT2 * smoothT * cp1.z + 3.0f * invT * t2 * cp2.z + t3 * end.z;
...
{
    float speedMultiplier = 1.0f / (static_cast<float>(speed) * 0.3f);
    speedMultiplier = std::clamp(speedMultiplier, 0.1f, 3.0f);
...
{
    return glm::degrees(m_fieldOfView);
}
...
void Camera::SetFieldOfView(float fovDegrees)
{
    ...
    projectionMatrix = glm::perspective(m_fieldOfView, aspectRatio, m_nearPlane, m_farPlane);
...
void Camera::SetNearFarPlanes(float nearPlane, float farPlane)
{
    ...
    m_farPlane       = farPlane;
    projectionMatrix = glm::perspective(glm::radians(currentFov), aspectRatio, nearPlane, farPlane);
...
void Camera::MoveAroundTarget(bool x, bool y, bool z, bool continuous)
{
    ...
    glm::vec3 distVec    = position - target;
    m_rotationDistance   = glm::length(distVec);
...
float Camera::GetRotationProgress() const
{
    ...
    if (m_rotateAroundX && m_targetRotationX > 0.0f) { totalProgress += (m_currentRotationX / m_targetRotationX); activeAxes++; }
    if (m_rotateAroundY && m_targetRotationY > 0.0f) { totalProgress += (m_currentRotationY / m_targetRotationY); activeAxes++; }
    if (m_rotateAroundZ && m_targetRotationZ > 0.0f) { totalProgress += (m_currentRotationZ / m_targetRotationZ); activeAxes++; }

    if (activeAxes > 0) totalProgress /= activeAxes;
...
glm::vec3 Camera::CalculateRotationPosition(float angleX, float angleY, float angleZ) const
{
    ...
    glm::vec3 rotated = glm::vec3(combined * glm::vec4(relPos, 0.0f));
    return m_rotationTarget + rotated;
}
...
{
    try {
        float fwdLen   = glm::length(forwardVec);
        float rightLen = glm::length(rightVec);
        float upLen    = glm::length(upVec);

        if (fwdLen < 0.001f || rightLen < 0.001f || upLen < 0.001f) {
...
void Camera::CalculateDirectionVectors(float yaw, float pitch, glm::vec3& outForward, glm::vec3& outRight, glm::vec3& outUp) const
{
    try {
        ...
        float cosPitch = FAST_COS(pitch);
        float sinPitch = FAST_SIN(pitch);
        float cosYaw   = FAST_COS(yaw);
        float sinYaw   = FAST_SIN(yaw);

        outForward = glm::normalize(glm::vec3(cosPitch * sinYaw, sinPitch, cosPitch * cosYaw));
    ...
    glm::vec3& outForward, glm::vec3& outRight, glm::vec3& outUp)
{
    try {
        ...
        float cosPitch = FAST_COS(newPitch);
        float sinPitch = FAST_SIN(newPitch);
        float cosYaw   = FAST_COS(newYaw);
        float sinYaw   = FAST_SIN(newYaw);
