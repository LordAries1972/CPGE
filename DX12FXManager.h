...
//-------------------------------------------------------------------------------------------------
// DX12FXManager.h - Visual FX Queue and Parallax Scroll Effects — DirectX 12 Pipeline
//
// This is the DX12-native implementation of FXManager.
// DX_FXManager.h redirects here automatically for __USE_DIRECTX_12__ builds.
//-------------------------------------------------------------------------------------------------
#include "Includes.h"
#include "Renderer.h"
...
// ---- Enumerations (shared with DX_FXManager.h — identical definitions) ----
...
struct WarpTunnelData {
    ...
    XMFLOAT3 smoothLookTarget = { 0.0f, 0.0f, 900.0f };
...
// Per-effect state for ImageFadeStrobe
...
struct ActiveFXState {
        ...
        , textScrollerActive(false), fadeEffectActive(false), scrollEffectsActive(false)
    {
        ...
        activeScrollTextures.clear();
        textScrollerIDs.reserve(20);
        activeScrollTextures.reserve(10);
    }
...
struct CallbackEntry {
    ...
    CallbackEntry(CallbackEntry&& other) noexcept
        : fxID(other.fxID), callback(std::move(other.callback)),
          isExecuted(other.isExecuted.load()), creationTime(other.creationTime) {
        other.fxID = -1; other.isExecuted = true;
    }
...
private:
    ...
    std::vector<int> CollectActiveTextScrollerIDs() const {
        ...
        activeIDs.reserve(effects.size());
        for (const auto& fx : effects) {
            if (fx.type == FXType::TextScroller)
                activeIDs.push_back(fx.fxID);
        }
        return activeIDs;
    }
    ...
    std::vector<BlitObj2DIndexType> CollectActiveScrollTextures(const std::vector<BlitObj2DIndexType>& texturesToCheck) const {
        ...
        activeTextures.reserve(texturesToCheck.size());
        for (BlitObj2DIndexType textureIndex : texturesToCheck) {
            ...
            if (hasActiveScrollEffect)
                activeTextures.push_back(textureIndex);
        }
    ...
    void SafelyClearAllEffects() {
        try {
            ...
            std::vector<CallbackEntry> tempCallbacks;
            tempCallbacks.swap(pendingCallbacks);
        }
        catch (const std::exception& e) {
            debug.logLevelMessage(LogLevel::LOG_ERROR, L"[DX12FXManager] Exception in SafelyClearAllEffects: " +
    ...
    // Effects queued while bIsRendering==true (e.g. from a FadeOutThenCallback callback
    // that fires inside Render()).  push_back during iteration triggers vector reallocation
    // which crashes the Particle sub-vector's move constructor under MSVC debug iterators.
    // These are flushed into 'effects' at the end of Render(), after all iteration is done.
    std::vector<FXItem> m_pendingEffects;

    // DX12 uses D2D (via DX12Renderer's 11on12 layer) for 2D rendering — no raw DX11 resources.
    // Color fades are drawn with renderer->DrawRectangle(); no PSO or vertex buffer is needed here.
};
