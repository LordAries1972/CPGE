...
#ifdef _DEBUG
    WriteCallStackLog(details.exceptionDescription, details.exceptionCode, details.exceptionAddress,
        details.threadId, details.processId,
        details.stackFrames.get(), details.frameCount);
#endif
...
#ifdef _DEBUG
// Returns the platform-appropriate writable path for the call-stack log file.
void ExceptionHandler::GetCallStackLogPath(char* outPath, size_t pathSize) {
    if (!outPath || pathSize == 0) return;

#if defined(_WIN32) || defined(_WIN64)
    // Write next to the executable (process working directory)
    snprintf(outPath, pathSize, "%s", CALLSTACK_LOG_FILENAME);
#elif defined(__ANDROID__)
    // Android: /data/local/tmp is writable without root in debug/development builds
    snprintf(outPath, pathSize, "/data/local/tmp/%s", CALLSTACK_LOG_FILENAME);
#elif defined(TARGET_OS_IPHONE) || defined(TARGET_IPHONE_SIMULATOR)
    // iOS: $TMPDIR points to the app sandbox temp folder (guaranteed writable)
    const char* tmpDir = getenv("TMPDIR");
    if (tmpDir && strlen(tmpDir) > 0) {
        snprintf(outPath, pathSize, "%s/%s", tmpDir, CALLSTACK_LOG_FILENAME);
    } else {
        snprintf(outPath, pathSize, "/tmp/%s", CALLSTACK_LOG_FILENAME);
    }
#elif defined(__APPLE__)
    // macOS: write next to the binary (process working directory)
    snprintf(outPath, pathSize, "%s", CALLSTACK_LOG_FILENAME);
#elif defined(__linux__)
    // Linux: write next to the binary (process working directory)
    snprintf(outPath, pathSize, "%s", CALLSTACK_LOG_FILENAME);
#else
    // Unknown platform: current directory
    snprintf(outPath, pathSize, "%s", CALLSTACK_LOG_FILENAME);
#endif
}

// Writes the call-stack report to CALLSTACK_LOG_FILENAME immediately after a crash.
// Called automatically from all exception/signal handlers — never call directly.
//
// NOTE: When called from a Unix signal handler the process is already fatally wounded.
// fopen/fprintf are not async-signal-safe but are the only portable option here, and
// the process is terminating so a torn write is acceptable for a debug log.
void ExceptionHandler::WriteCallStackLog(const char* exceptionDescription,
                                          uint32_t exceptionCode,
                                          uint64_t exceptionAddress,
                                          uint32_t threadId,
                                          uint32_t processId,
                                          const StackFrameInfo* frames,
                                          int frameCount)
{
    char logPath[512];
    GetCallStackLogPath(logPath, sizeof(logPath));

    FILE* f = fopen(logPath, "w");
    if (!f) return; // Cannot open — silently skip (we're already crashing)

    // ---- Date/time string ----
    char dateTimeStr[64] = "Unknown";
#if defined(_WIN32) || defined(_WIN64)
    SYSTEMTIME st;
    GetLocalTime(&st);
    ...
    if (tmInfo) strftime(dateTimeStr, sizeof(dateTimeStr), "%Y-%m-%d %H:%M:%S", tmInfo);
#endif

    // ---- Platform / architecture strings (one branch compiled per target) ----
#if defined(_WIN32) || defined(_WIN64)
    const char* platformStr = "Windows";
#elif defined(__ANDROID__)
    const char* platformStr = "Android";
#elif defined(TARGET_OS_IPHONE) || defined(TARGET_IPHONE_SIMULATOR)
    const char* platformStr = "iOS";
#elif defined(__APPLE__)
    const char* platformStr = "macOS";
#elif defined(__linux__)
    ...
    // ================================================================
    fprintf(f, "===========================================================\n");
    fprintf(f, "  EXCEPTION CALL-STACK LOG\n");
    fprintf(f, "  Generated  : %s\n", dateTimeStr);
    fprintf(f, "===========================================================\n");
    fprintf(f, "  Exception  : %s\n", exceptionDescription ? exceptionDescription : "Unknown");
    if (exceptionCode != 0) {
    ...
    if (logFrames > 0) {
        fprintf(f, " (%d of %d frames captured, showing up to %d)\n",
                logFrames, frameCount, MAX_LOG_FRAMES);
    } else {
    ...
    if (logFrames > 0) {
        for (int i = 0; i < logFrames; ++i) {
            const StackFrameInfo& fr = frames[i];

            fprintf(f, "  #%02d  Address  : 0x%016llX\n", i, (unsigned long long)fr.address);

            if (strlen(fr.functionName) > 0) {
                if (fr.displacement > 0) {
                    fprintf(f, "       Function : %s  +0x%llX\n",
                            fr.functionName, (unsigned long long)fr.displacement);
                } else {
                    fprintf(f, "       Function : %s\n", fr.functionName);
                }
            }

            if (strlen(fr.moduleName) > 0) {
                fprintf(f, "       Module   : %s\n", fr.moduleName);
            }

            if (strlen(fr.fileName) > 0 && fr.lineNumber > 0) {
                fprintf(f, "       Source   : %s  Line %u\n", fr.fileName, fr.lineNumber);
            }
        ...
        fprintf(f, "  (Stack frames unavailable — no debug symbols or exception context)\n");
    }
    ...
    bool hasValidCalls = false;
    for (int i = 0; i < LAST_CALLS_BUFFER_SIZE; ++i) {
        int idx = (m_lastCallsIndex + i) % LAST_CALLS_BUFFER_SIZE;
        if (strlen(m_lastCalls[idx]) > 0) {
            fprintf(f, "  %2d.  %s\n", i + 1, m_lastCalls[idx]);
            hasValidCalls = true;
        }
    }
    ...
    debug.logDebugMessage(LogLevel::LOG_INFO,
        L"[ExceptionHandler] Call-stack log written to: %hs", logPath);
}
#endif // _DEBUG
