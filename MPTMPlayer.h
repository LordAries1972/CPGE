...
#define MPTM_BUFFER_SIZE (44100 * 2 * 2)
...
static_assert(sizeof(MPTMHeader)       == 192, "MPTMHeader must be exactly 192 bytes");
static_assert(sizeof(MPTMSampleHeader) ==  80, "MPTMSampleHeader must be exactly 80 bytes");
...
public:
    ...
    bool LoadMPTMFile(const std::wstring& filename);
...
private:
    ...
    std::vector<MPTMInstrument> instruments;
    std::vector<MPTMSample> samples;
    ...
    void SilenceBuffer();
    bool LoadExternalSample(const std::wstring& path, MPTMSample& sample);
    void UnpackPatterns();
    void TickRow();
    ...
    void ApplyVibrato(MPTMChannelVoice& voice, bool fine);
    void ApplyTremolo(MPTMChannelVoice& voice);
    void ApplyPanbrello(MPTMChannelVoice& voice);
