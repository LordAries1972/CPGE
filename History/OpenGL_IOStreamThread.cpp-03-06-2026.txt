...
void OpenGLRenderer::LoaderTaskThread()
{
           ...
           (status != ThreadStatus::Stopped))
    {
        ...
        switch (scene.stSceneType)
        {
            ...
            {
                threadManager.threadVars.bLoaderTaskFinished.store(false);
                // Stage-announcement lambda (mirrors DX11 IOStreamThread showStage pattern)
                ...
                if (!wasResizing.load())
                {
                    ...
                    // illuminates the ship's aft face, creating the title-screen rim/halo.
            ...
            case SceneType::SCENE_GAMEPLAY:
            {
                ...
                // On first load: create lights, parse scene, load music, setup camera.
                // On resize: reload 2D textures and re-fit the camera to the new viewport.
                if (!wasResizing.load())
                {
                    showStage(L"Initialising lighting...");
                    LightStruct sunLight{};
                ...
                {
                    // Resize path: refit camera to new viewport dimensions (mirrors DX11 resize branch)
                    showStage(L"Updating display...");
                    {
                        float camW, camH;
                        switch (config.myConfig.displayMode)
            ...
            {
                threadManager.threadVars.bLoaderTaskFinished.store(false);
                // Default behaviour for unhandled scene types: load all textures, then pause.
                threadManager.threadVars.bLoaderTaskFinished.store(true);
                break;
            }
        }
