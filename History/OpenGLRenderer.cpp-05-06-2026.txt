...
void main() {
    ...
    if (uLightCount > 0) {
        for (int i = 0; i < uLightCount && i < 8; ++i) {
            ...
            float roughFactor  = max(1.0 - uRoughness, 0.0);
...
void OpenGLRenderer::LoadShaders()
{
    ...
    // 3D shader
    GLuint prog3D = CreateShaderProgram(k_3dVertGLSL, k_3dFragGLSL);
    if (prog3D) {
        ...
        // Cache all uniform locations now — avoid glGetUniformLocation every frame
        auto& u = m_uniforms3D;
        u.uModel          = glGetUniformLocation(prog3D, "uModel");
        u.uView           = glGetUniformLocation(prog3D, "uView");
        u.uProjection     = glGetUniformLocation(prog3D, "uProjection");
        u.uViewPos        = glGetUniformLocation(prog3D, "uViewPos");
        u.uAlpha          = glGetUniformLocation(prog3D, "uAlpha");
        u.uLightCount     = glGetUniformLocation(prog3D, "uLightCount");
        u.uDiffuse        = glGetUniformLocation(prog3D, "uDiffuse");
        u.uNormalMap      = glGetUniformLocation(prog3D, "uNormalMap");
        u.uHasDiffuse     = glGetUniformLocation(prog3D, "uHasDiffuse");
        u.uHasNormalMap   = glGetUniformLocation(prog3D, "uHasNormalMap");
        u.uMetallic       = glGetUniformLocation(prog3D, "uMetallic");
        u.uRoughness      = glGetUniformLocation(prog3D, "uRoughness");
        u.uNormalScale    = glGetUniformLocation(prog3D, "uNormalScale");
        u.uKd             = glGetUniformLocation(prog3D, "uKd");
        u.uKa             = glGetUniformLocation(prog3D, "uKa");
        u.uKs             = glGetUniformLocation(prog3D, "uKs");
        u.uNs             = glGetUniformLocation(prog3D, "uNs");
        u.uEmissiveFactor = glGetUniformLocation(prog3D, "uEmissiveFactor");
        u.uEmissiveStr    = glGetUniformLocation(prog3D, "uEmissiveStrength");
        u.uLightDir       = glGetUniformLocation(prog3D, "uLightDir");
        u.uLightColor     = glGetUniformLocation(prog3D, "uLightColor");
        u.uAmbient        = glGetUniformLocation(prog3D, "uAmbient");
        for (int li = 0; li < 8; ++li) {
            std::string b = "uLights[" + std::to_string(li) + "].";
            u.lights[li].position  = glGetUniformLocation(prog3D, (b + "position").c_str());
            u.lights[li].direction = glGetUniformLocation(prog3D, (b + "direction").c_str());
            u.lights[li].color     = glGetUniformLocation(prog3D, (b + "color").c_str());
            u.lights[li].ambient   = glGetUniformLocation(prog3D, (b + "ambient").c_str());
            u.lights[li].intensity = glGetUniformLocation(prog3D, (b + "intensity").c_str());
            u.lights[li].range     = glGetUniformLocation(prog3D, (b + "range").c_str());
            u.lights[li].innerCone = glGetUniformLocation(prog3D, (b + "innerCone").c_str());
            u.lights[li].outerCone = glGetUniformLocation(prog3D, (b + "outerCone").c_str());
            u.lights[li].type      = glGetUniformLocation(prog3D, (b + "type").c_str());
            u.lights[li].active    = glGetUniformLocation(prog3D, (b + "active").c_str());
        }
        u.populated = true;
    }
