...
namespace VulkanModelUtils
{
    ...
    // queueMutex must be the renderer's queue mutex so this loader-thread submit
    // doesn't race with the render-thread submit in RenderFrame().
    ...
    // queueMutex must be the renderer's queue mutex when called from a loader thread.
    ...
    // Create a solid-colour texture (any size; defaults to 1×1 for fallback use).
    // queueMutex must be the renderer's queue mutex when called from a loader thread.
