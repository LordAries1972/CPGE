// ---------------------------------------------------------------------------------------------------------------
// OpenGLFXManager.h  —  Visual Effects Manager for the OpenGL Renderer
// ---------------------------------------------------------------------------------------------------------------
// OpenGL equivalent of DX_FXManager.h.
// Replaces all D3D11 types with OpenGL equivalents:
//   ID3D11DeviceContext*  ->  (none; OpenGL is a state-machine — no command buffer)
//   ID3D11BlendState*     ->  GLuint shader program + glBlendFunc state
//   XMMATRIX              ->  per-platform matrix type (DirectXMath / GLM)
//
// Platform guards:
//   #if defined(_WIN32)  /  #elif defined(__linux__)  /  #elif defined(__ANDROID__)
// ---------------------------------------------------------------------------------------------------------------
#pragma once
...
// Platform-specific math / OpenGL headers
...
struct GLParticle {
    ...
    float angle = 0.0f, speed = 0.0f, radius = 0.0f, maxRadius = 0.0f;
    float vx = 0.0f, vy = 0.0f;
    int   delayBase = 0, delayCount = 0;
    bool  completed = false, hasLoggedCompletion = false;
};

struct GLStar {
    // position.x/y = 2D world spread position (matches DX11 Particle p.x/p.y)
    // position.z   = z-depth / angle (matches DX11 Particle p.angle)
    XMFLOAT3 position = { 0.0f, 0.0f, 0.0f };
    XMFLOAT4 color    = { 1.0f, 1.0f, 1.0f, 1.0f };
    float     size    = 1.0f;   // visual radius (matches DX11 Particle p.radius)
    float     speed   = 1.0f;
    // Spread-direction vectors for reverse-mode convergence (matches DX11 p.vx/p.vy)
    float     vx      = 0.0f;
    float     vy      = 0.0f;
    float     maxRadius = 0.0f; // resetDepth reference for reset logic
    bool      active  = true;
};
...
struct GLWarpTunnelData {
    ...
    XMFLOAT3 smoothLookTarget = { 0.0f, 0.0f, 800.0f };
    ...
    std::vector<GLTunnelRing> rings;
};

struct GLTextScrollData {
    std::wstring             text;
    ...
    float regionWidth        = 800.0f;
    float regionHeight       = 600.0f;
    float regionX            = 0.0f;
    float regionY            = 0.0f;
...
// Default loading-screen font resolved per platform
...
// ---------------------------------------------------------------------------------------------------------------
// Fireworks effect data structures (no GL prefix — each renderer header compiles independently)
// ---------------------------------------------------------------------------------------------------------------
...
struct FireworkParticle {
    float x         = 0.0f;
    float y         = 0.0f;
    float angle     = 0.0f;                                                     // Azimuth angle in radians (reference)
    float screenDX  = 0.0f;                                                     // 2D screen-space X direction (Fibonacci sphere projection)
    float screenDY  = 0.0f;                                                     // 2D screen-space Y direction (Fibonacci sphere projection)
...
// Top-level data block stored inside GLFXItem for a running fireworks effect
...
// Per-effect state for ImageFadeStrobe (GL build)
struct GLImageFadeStrobeData {
    ...
    float              fadeOverTime   = 1.0f;                                    // Seconds to complete one phase
...
struct GLFXItem {
    ...
    std::vector<GLParticle> particles;

    // Starfield
    XMFLOAT3 starfieldOrigin  = { 0.0f, 0.0f, 0.0f };
    bool     starfieldReverse = false;
    std::vector<GLStar> stars;

    // Text scroller
    GLTextScrollData textScrollData;

    // WarpDotTunnel
    GLWarpTunnelData warpTunnelData;
    ...
    GLImageFadeStrobeData imageFadeStrobeData;                                     // State for the ImageFadeStrobe effect (only used when type == ImageFadeStrobe)
};
...
struct GLActiveFXState {
    ...
    GLActiveFXState() { textScrollerIDs.reserve(20); activeScrollTextures.reserve(10); }
};

struct GLCallbackEntry {
    int                    fxID = -1;
    std::function<void()>  callback;
    std::atomic<bool>      isExecuted{ false };
    std::chrono::steady_clock::time_point creationTime;

    GLCallbackEntry() { creationTime = std::chrono::steady_clock::now(); }
    GLCallbackEntry(int id, std::function<void()> cb)
        : fxID(id), callback(std::move(cb)) { creationTime = std::chrono::steady_clock::now(); }
    GLCallbackEntry(const GLCallbackEntry& o)
        : fxID(o.fxID), callback(o.callback),
          isExecuted(o.isExecuted.load()), creationTime(o.creationTime) {}
    GLCallbackEntry& operator=(const GLCallbackEntry& o) {
        if (this != &o) { fxID = o.fxID; callback = o.callback;
            isExecuted = o.isExecuted.load(); creationTime = o.creationTime; }
        return *this;
    }
    GLCallbackEntry(GLCallbackEntry&& o) noexcept
        : fxID(o.fxID), callback(std::move(o.callback)),
          isExecuted(o.isExecuted.load()), creationTime(o.creationTime)
    { o.fxID = -1; o.isExecuted = true; }
    GLCallbackEntry& operator=(GLCallbackEntry&& o) noexcept {
        if (this != &o) {
            fxID = o.fxID; callback = std::move(o.callback);
            isExecuted = o.isExecuted.load(); creationTime = o.creationTime;
            o.fxID = -1; o.isExecuted = true;
...
public:
    ...
    std::vector<GLFXItem> effects;

    void Initialize();
    void CleanUp();
    void AddEffect(const GLFXItem& fxItem);

    void StopAllFXForResize();
    void RestartFXAfterResize();

    // Primary render entry points
    void RenderBackground();  // call BEFORE 3D models: updates + renders starfield only
    void Render();            // call AFTER GUI: fades, tunnel, callbacks (skips starfield)
    void Render2D();
    ...
    void RenderStarfield(GLFXItem& fxItem, const XMMATRIX& viewMatrix);

    // WarpDotTunnel
    int              tunnelID       = 0;
    std::vector<int> m_tunnelSortBuf;   // reused each frame to avoid per-frame allocation
    ...
    void  RenderImageFadeStrobe(BlitObj2DIndexType type, int x, int y, int w, int h);                 // Blit image at current strobe alpha
    ...
    void RenderParticles(GLFXItem& fxItem);
...
private:
    void UpdateFireworks(GLFXItem& fx);                                             // Advance timers, launch rockets, and update particle state each frame (called from Render2D)
    void DrawFireworksPixels(GLFXItem& fx);                                         // Internal draw pass — blits rockets and particles for one Fireworks FXItem

    // ImageFadeStrobe private helper
    void UpdateImageFadeStrobe(GLFXItem& fx, float deltaTime);                      // Advance strobe alpha each frame (called from Render2D)

    // WarpDotTunnel private helpers
    void UpdateWarpDotTunnel(GLFXItem& fx, float deltaTime);
    void RenderWarpDotTunnel(GLFXItem& fx);
    ...
    void ApplyScroller(GLFXItem& fxItem);
    ...
    void SafelyClearAllEffects() {
        ...
        std::vector<GLCallbackEntry> tmpc; tmpc.swap(m_pendingCallbacks);
    }

    // OpenGL resources for fullscreen fade quad
    ...
    mutable std::recursive_mutex    m_effectsMutex;
    std::vector<GLCallbackEntry> m_pendingCallbacks;
    std::vector<GLScrollTween>   m_activeTweens;
    GLActiveFXState              m_savedFXState;

    // Full-effect snapshot for scene transitions
    std::vector<GLFXItem> m_sceneSavedEffects;
