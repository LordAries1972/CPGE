...
void DX12Renderer::RenderFrame()
{
    ...
    try
    {
        ...
        RECT           rc;
            ...
            // STEP 6: Clear render targets with thread-safe access
            try
            {
                ...
                {
                    #if defined(_DEBUG_DX12RENDERER_) && defined(_DEBUG)
                        debug.logLevelMessage(LogLevel::LOG_WARNING, L"[DX12 RENDERFRAME] Could not acquire D2D lock for clearing - skipping clear");
                    #endif
                }
            }
            catch (const std::exception& e)
            {
                #if defined(_DEBUG_DX12RENDERER_) && defined(_DEBUG)
                    debug.logDebugMessage(LogLevel::LOG_ERROR, L"[DX12 RENDERFRAME] Clear operation failed: %hs", e.what());
                #endif
                threadManager.threadVars.bIsRendering.store(false);
                #ifdef RENDERER_IS_THREAD
                    continue;
                ...
                m_wrappedBackBuffers[m_frameIndex] && m_d2dRenderTargets[m_frameIndex])
            {
                ...
                // ── PASS 1: Background images ─────────────────────────────────────
                // Draws scene-specific background behind all overlay content.
                ...
                // ── PASS 2: Scene-specific 2D + overlays ─────────────────────────
                ThreadLockHelper d2dRenderLock(threadManager, D2DLockName, 100);
                if (d2dRenderLock.IsLocked())
                {
                    #if defined(_DEBUG_DX12RENDERER_) && defined(_DEBUG)
                        debug.logLevelMessage(LogLevel::LOG_DEBUG, L"[DX12 RENDERFRAME] Beginning 2D overlay pass");
                    #endif
                    ...
                    if (USE_RENDERER_INFO && !scene.bSceneSwitching && m_d2dContext && m_dwriteFactory)
                    {
                        ...
                        if (riShow)
                        {
                            ...
                            ComPtr<IDWriteTextFormat> riFmt;
                            HRESULT riHr = m_dwriteFactory->CreateTextFormat(
                                FontName, nullptr,
                                DWRITE_FONT_WEIGHT_NORMAL, DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH_NORMAL,
                                riFontSize, L"en-us", &riFmt);

                            if (SUCCEEDED(riHr) && riFmt)
                            {
                                ComPtr<IDWriteTextLayout> riLayout;
                                riHr = m_dwriteFactory->CreateTextLayout(
                    ...
                    // End D2D overlay pass + post-processing FX
                    try
                    {
                ...
                {
                    // D2D lock not acquired — still need EndDraw to pair with BeginDraw above
                    m_d2dContext->EndDraw();
                    #if defined(_DEBUG_DX12RENDERER_) && defined(_DEBUG)
                        debug.logLevelMessage(LogLevel::LOG_WARNING, L"[DX12 RENDERFRAME] Could not acquire D2D render lock - skipping 2D overlays");
                    #endif
                }
...
inline void DX12Renderer::RenderGamePlay(float deltaTime)
{
    ...
    if (threadManager.threadVars.bLoaderTaskFinished.load())
    {
        ...
        for (int i = 0; i < MAX_SCENE_MODELS; ++i)
        {
            ...
            {
                // Push current camera matrices and state into the model info so the model
                // can compute its world-view-projection during Render().
                ...
                // Render via the DX11-on-DX12 compatibility context.
                // The DX11 device context is layered over the DX12 command queue, so
                // draw calls issued here are translated to DX12 commands automatically.
