...
{
    // Left-handed perspective, depth [0,1] (Vulkan ZO convention).
    // Computed manually to avoid GLM extension availability uncertainty.
    // LH convention: w_clip = +z_view, so front objects (z_view > 0) have positive w → correct perspective divide.
    ...
    proj[2][3] =  1.0f;                                          // w_clip = z_view (LH)
    proj[3][2] = -(farPlane * nearPlane) / (farPlane - nearPlane);

    proj[1][1] *= -1.0f;   // Flip Y for Vulkan Y-down NDC
    return proj;
}
    ...
    try {
        ...
        if (progress >= 1.0f)
        {
            ...
            if (m_focusOnTarget)
            {
                ...
                m_pitch = newPitch;
                up = glm::vec3(0.0f, 1.0f, 0.0f);
                viewMatrix = glm::lookAtLH(position, m_originalTarget, up);
            }
            else
            {
...
void Camera::SetupDefaultCamera(float windowWidth, float windowHeight)
{
    ...
    SetViewMatrix(glm::lookAtLH(eye, lookAt, upVec));
...
void Camera::UpdateContinuousRotation()
{
    ...
    viewMatrix = glm::lookAtLH(position, m_rotationTarget, up);
