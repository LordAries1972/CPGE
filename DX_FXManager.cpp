...
void FXManager::AddEffect(const FXItem& fxItem) {
    ...
    // Defer push_back if Render() is currently iterating effects to prevent
    // reallocation-induced heap corruption from concurrent access.
    if (bIsRendering.load()) {
        std::lock_guard<std::mutex> lock(m_effectsMutex);
        m_pendingEffects.push_back(std::move(newEffect));
    } else {
...
void FXManager::StopAllFXForResize()
{
    ...
    try {
        ...
        // Save and stop Fireworks if active
        if (fireworksID > 0) {
            try {
                ...
                StopFireworks();
#if defined(_DEBUG_FXMANAGER_)
                debug.logLevelMessage(LogLevel::LOG_INFO, L"[FXManager] Fireworks stopped for resize");
#endif
            }
            catch (const std::exception& e) {
                debug.logLevelMessage(LogLevel::LOG_ERROR, L"[FXManager] Exception stopping Fireworks: " +
                    std::wstring(e.what(), e.what() + strlen(e.what())));
            }
        }
...
void FXManager::RemoveCompletedEffects() {
    ...
    for (size_t i = 0; i < effects.size(); ++i) {
        ...
        // TextFadeInOut is managed by RenderLoadingText/StopLoadingText — never timeout-remove it;
        // only clean it up once UpdateTextFadeInOut has set progress=1.0 (Stopped or immediateStop).
        ...
        // Fireworks runs indefinitely; only remove when StopFireworks() sets progress=1.0.
...
void FXManager::RenderFullScreenQuad(const XMFLOAT4& color) {
    ...
    try
    {
        ...
        // Explicitly cover the full render target — D2D's EndDraw can leave the
        // viewport in an undefined state, so we must not inherit whatever it left.
        D3D11_VIEWPORT fullVP = {};
        fullVP.Width    = static_cast<float>(renderer->iOrigWidth);
        fullVP.Height   = static_cast<float>(renderer->iOrigHeight);
...
void FXManager::Render() {
    ...
    try {
        ...
        // Flush effects that were deferred by AddEffect() while bIsRendering was true.
        // Done AFTER all iteration so push_back never invalidates live iterators.
        {
            std::lock_guard<std::mutex> lock(m_effectsMutex);
...
// =============================================================================
// TextFadeInOut — loading-screen text with frame-accurate fade in / out
// =============================================================================

// -----------------------------------------------------------------------------
// ShowLoadingText — start a new text fade-in.
//   If any TextFadeInOut effect is already active it is immediately set to fade
//   out; the new text waits that long before starting its own fade-in.
// -----------------------------------------------------------------------------
int FXManager::ShowLoadingText(const std::wstring& text,
                                XMFLOAT4 endColor,
                                ...
                                const TextRenderStyle* fontStyle)
{
    ...
    // Signal all existing TextFadeInOut effects to fade out from their current actual color
    for (auto& fx : effects) {
        ...
        if (d.pendingDelay > 0.0f) {
            // Effect has not started rendering yet — kill immediately, no visible content to fade
            d.immediateStop = true;
            fx.progress = 1.0f;
            continue;
        }

        if (d.phase == TextFadePhase::FadeIn) {
            // Record actual current color so FadeOut starts there, not at full endColor.
            // If the effect was barely visible (alpha≈0), fadeOutStartColor≈transparent and
            // RenderTextFadeInOut's existing 0.005 guard will skip drawing — no blip.
            float t = (d.fadeInDuration > 0.0f)
                      ? std::clamp(d.phaseTimer / d.fadeInDuration, 0.0f, 1.0f)
                      : 1.0f;
            d.fadeOutStartColor = {
            ...
            d.phase      = TextFadePhase::FadeOut;
            d.phaseTimer = 0.0f;
            if (remaining > maxRemainingFadeOut)
                maxRemainingFadeOut = remaining;
    ...
    fx.fxID        = newID;
    fx.nextEffectID = -1;
    fx.type        = FXType::TextFadeInOut;
    fx.subtype     = FXSubType::TXT_FADE_IN;
    fx.duration    = 0.0f;
    fx.progress    = 0.0f;
    fx.delay       = 0.0f;
    fx.timeout     = 0.0f;
    ...
    // Auto-position: left-aligned at 70% screen height by default
...
// -----------------------------------------------------------------------------
// StopLoadingText — gracefully fade out all TextFadeInOut effects.
//   Effects that have not started yet (pendingDelay > 0) are killed immediately.
//   Effects in FadeIn/Holding are transitioned to FadeOut from their current color.
//   Effects already in FadeOut are left to complete naturally.
// -----------------------------------------------------------------------------
void FXManager::StopLoadingText()
{
    for (auto& fx : effects) {
        ...
        // FadeOut: let the animation complete — RemoveCompletedEffects will clean it up
    }
}

// -----------------------------------------------------------------------------
// RenderLoadingText — update + render all TextFadeInOut effects.
//   Must be called from inside an active D2D draw context (RenderBackgroundImage).
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// UpdateTextFadeInOut — advance the phase state machine for one effect
// -----------------------------------------------------------------------------
void FXManager::UpdateTextFadeInOut(FXItem& fx, float deltaTime)
{
    ...
    // Honour pending delay before starting FadeIn (used for cross-fade)
    if (d.pendingDelay > 0.0f) {
        d.pendingDelay -= deltaTime;
        return;
    }
    ...
    case TextFadePhase::FadeIn:
        if (d.phaseTimer >= d.fadeInDuration) {
            d.phase      = TextFadePhase::Holding;
            d.phaseTimer = 0.0f;
        }
        break;
    ...
    case TextFadePhase::FadeOut:
        if (d.phaseTimer >= d.fadeOutDuration) {
            d.phase      = TextFadePhase::Stopped;
            fx.progress  = 1.0f;
        }
        break;
...
// -----------------------------------------------------------------------------
// RenderTextFadeInOut — draw one effect at its current alpha
// -----------------------------------------------------------------------------
void FXManager::RenderTextFadeInOut(FXItem& fx)
{
    ...
    // Calculate interpolated colour + alpha for the current phase
    ...
    case TextFadePhase::Holding:
        renderColor = d.endColor;
        alpha       = d.endColor.w;
        break;
    case TextFadePhase::FadeOut: {
        float t = (d.fadeOutDuration > 0.0f)
                  ? std::clamp(d.phaseTimer / d.fadeOutDuration, 0.0f, 1.0f)
                  : 1.0f;
    ...
    if (renderColor.w < 0.005f) return;  // Fully transparent — skip draw
...
// HasActiveLoadingTextEffects — returns true if any TextFadeInOut effect is still animating.
//   Used by the render frame to continue calling RenderLoadingText after bLoaderTaskFinished
//   so graceful fade-outs play through even after loading completes.
...
// ============================================================================================================
// StartFireworks — begins a continuous fireworks display; rockets launch at freqRate-second intervals.
// Parameters:
//   freqRate - seconds between each new rocket launch (clamped to a minimum of 0.1s)
// ============================================================================================================
void FXManager::StartFireworks(float freqRate)
{
    ...
    debug.logLevelMessage(LogLevel::LOG_INFO, L"[FXManager] Fireworks started: FreqRate=" +
...
void FXManager::StopFireworks()
{
    ...
    debug.logLevelMessage(LogLevel::LOG_INFO, L"[FXManager] Fireworks stopped.");
    fireworksID = 0;
}
...
void FXManager::UpdateFireworks(FXItem& fx)
{
    ...
    // Advance launch timer; fire a new rocket when the interval elapses and under the 10-rocket cap
    fw.launchTimer += dt;
