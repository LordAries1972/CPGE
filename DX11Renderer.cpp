...
void DX11Renderer::CreateDeviceAndSwapChain(HWND hwnd) {
    ...
    // Enable D3D11 immediate-context multi-thread protection at device creation time.
    // The loader thread (LoaderTaskThread) accesses D3D11 resources concurrently with
    // the render thread from SCENE_INTRO onward — before OpenMovie() is ever called.
    // Without this, the D3D11 debug layer (D3D11_3SDKLayers.dll) raises SEH 0x87D
    // ("same time" concurrent context access) as soon as any two threads touch the
    // immediate context simultaneously, even if D2D's MULTI_THREADED factory serialises
    // its own internal calls. Must be set before any loader thread activity, not deferred.
    {
        ComPtr<ID3D10Multithread> pMultithread;
        if (SUCCEEDED(m_d3dDevice.As(&pMultithread)))
        {
            pMultithread->SetMultithreadProtected(TRUE);
            debug.logLevelMessage(LogLevel::LOG_INFO, L"D3D11 immediate-context multi-thread protection enabled at device creation");
        }
    }
...
//    swapDesc.SwapEffect = DXGI_SWAP_EFFECT_FLIP_DISCARD;
    ...
    swapDesc.Flags = DXGI_SWAP_CHAIN_FLAG_ALLOW_MODE_SWITCH;  // Required: lets ResizeTarget actually switch the display mode in exclusive fullscreen
...
// DrawVideoFrameFromCPU — pure D2D path, no D3D11 context calls.
// UpdateVideoTexture() populates m_cpuFrameBuffer before the D2D BeginDraw block;
// this function just updates the cached D2D bitmap and blits it. Safe inside BeginDraw.
void DX11Renderer::DrawVideoFrameFromCPU(const Vector2& position, const Vector2& size,
    const MyColor& tintColor, const BYTE* pPixels, UINT width, UINT height, UINT rowPitch)
{
    if (!pPixels || !m_d2dRenderTarget || width == 0 || height == 0)
    {
        debug.logLevelMessage(LogLevel::LOG_ERROR, L"DrawVideoFrameFromCPU: Invalid parameters");
        return;
    }

    try
    {
        // Reset cached D2D bitmap when video dimensions change.
        if (m_videoStagingW != width || m_videoStagingH != height)
        {
            m_videoBitmap.Reset();
            m_videoStagingW = width;
            m_videoStagingH = height;
        }

        HRESULT hr = S_OK;

        // Reuse the D2D bitmap across frames — update pixels via CopyFromMemory.
        // CopyFromMemory is a D2D-only operation, never calls the D3D11 context.
        if (!m_videoBitmap)
        {
            D2D1_BITMAP_PROPERTIES props = D2D1::BitmapProperties(
                D2D1::PixelFormat(DXGI_FORMAT_B8G8R8A8_UNORM, D2D1_ALPHA_MODE_PREMULTIPLIED));
            hr = m_d2dRenderTarget->CreateBitmap(
                ...
                pPixels, rowPitch, props, &m_videoBitmap);
        }
        else
        {
            D2D1_RECT_U updateRect = D2D1::RectU(0, 0, width, height);
            hr = m_videoBitmap->CopyFromMemory(&updateRect, pPixels, rowPitch);
        }

        if (FAILED(hr))
        {
            debug.logLevelMessage(LogLevel::LOG_ERROR, L"DrawVideoFrameFromCPU: Failed to update D2D bitmap");
            return;
        }
    ...
    {
        debug.logLevelMessage(LogLevel::LOG_ERROR, L"DrawVideoFrameFromCPU: Exception: " +
            std::wstring(e.what(), e.what() + strlen(e.what())));
    }
}
...
bool DX11Renderer::SetFullScreen(void)
{
    ...
    try {
        ...
        // Borderless fullscreen: query the monitor that owns the game window.
        // MonitorFromWindow is used (not GetContainingOutput) so that multi-monitor setups and
        // windows that span monitor boundaries are handled correctly.  The DXGI swap chain stays
        // windowed throughout — no SetFullscreenState(TRUE) is called — which avoids the
        // message-queue deadlock that exclusive DXGI fullscreen can trigger under DWM composition.
        // bSettingFullScreen is already true, so the WM_SIZE generated by SetWindowPos is
        // suppressed in WindowProc before it can re-enter the renderer.
        HMONITOR hMonitor = MonitorFromWindow(hwnd, MONITOR_DEFAULTTONEAREST);
        MONITORINFO monInfo = {};
        monInfo.cbSize = sizeof(MONITORINFO);
        if (!GetMonitorInfo(hMonitor, &monInfo))
        {
            debug.logLevelMessage(LogLevel::LOG_ERROR, L"[RENDERER] SetFullScreen: GetMonitorInfo failed");
        ...
        UINT fullscreenHeight = static_cast<UINT>(monInfo.rcMonitor.bottom - monInfo.rcMonitor.top);
            ...
            debug.logDebugMessage(LogLevel::LOG_DEBUG,
                L"[RENDERER] Borderless fullscreen target: %dx%d at monitor (%d,%d)",
        ...
        // WS_POPUP + SetWindowPos to cover the full monitor — recommended on Windows 10/11.
        HRESULT hr;
        ...
        if (FAILED(hr)) {
            debug.logLevelMessage(LogLevel::LOG_ERROR, L"[RENDERER] Failed to resize buffers for borderless fullscreen");
            threadManager.threadVars.bIsResizing.store(false);
            bFullScreenTransition.store(false);
            threadManager.threadVars.bSettingFullScreen.store(false);
            return false;
        ...
        hr = m_swapChain->GetBuffer(0, IID_PPV_ARGS(&backBuffer));
        if (FAILED(hr)) {
            debug.logLevelMessage(LogLevel::LOG_ERROR, L"[RENDERER] Failed to get back buffer after borderless resize");
            threadManager.threadVars.bIsResizing.store(false);
            bFullScreenTransition.store(false);
            threadManager.threadVars.bSettingFullScreen.store(false);
            return false;
        ...
        // Restore pipeline states cleared by ClearState() then recreate D2D resources
        SetupPipelineStates();
        CreateDirect2DResources();
...
//                          (delegates to SetFullScreen — the recommended default on Windows 10/11)
...
// This is the single call that main.cpp (and any other site) should use after renderer init.
// -------------------------------------------------------------------------------------------------
bool DX11Renderer::SetDisplayMode()
{
...
bool DX11Renderer::SetDisplayMode(DisplayMode mode, int width, int height, int refreshHz)
{
    switch (mode)
    {
        case DisplayMode::Windowed:
            return SetWindowedScreen();
        ...
        case DisplayMode::ExclusiveFullscreen:
        {
            if (width <= 0 || height <= 0)
            {
                debug.logLevelMessage(LogLevel::LOG_ERROR, L"[RENDERER] SetDisplayMode: invalid exclusive fullscreen dimensions");
                return false;
            }

            if (bFullScreenTransition.load())
            {
                debug.logLevelMessage(LogLevel::LOG_WARNING, L"[RENDERER] SetDisplayMode: fullscreen transition already in progress");
                return false;
            }
            ...
            try
            {
                ...
                // Obtain the DXGI output (monitor) that currently owns the game window.
                // Fall back to the adapter's first output when the swap chain is not yet
                // associated with an output (e.g. very early in startup).
                ComPtr<IDXGIOutput> output;
                HRESULT hr = m_swapChain->GetContainingOutput(&output);
                if (FAILED(hr) || !output)
                {
                    debug.logLevelMessage(LogLevel::LOG_WARNING,
                        L"[RENDERER] SetDisplayMode ExclusiveFS: GetContainingOutput failed — falling back to adapter output 0");
                    ComPtr<IDXGIDevice> dxgiDev;
                    if (SUCCEEDED(m_d3dDevice.As(&dxgiDev)))
                    {
                        ComPtr<IDXGIAdapter> dxgiAdapter;
                    ...
                    if (!output)
                    {
                        debug.logLevelMessage(LogLevel::LOG_ERROR, L"[RENDERER] SetDisplayMode ExclusiveFS: cannot obtain IDXGIOutput");
                        bFullScreenTransition.store(false);
                        threadManager.threadVars.bSettingFullScreen.store(false);
                        return false;
                    }
                ...
                // Find the closest supported display mode — avoids black-screen on unsupported resolutions.
                DXGI_MODE_DESC targetMode = {};
                targetMode.Width  = static_cast<UINT>(width);
                targetMode.Height = static_cast<UINT>(height);
                targetMode.Format = DXGI_FORMAT_B8G8R8A8_UNORM;
                ...
                if (FAILED(hr))
                {
                    debug.logLevelMessage(LogLevel::LOG_ERROR, L"[RENDERER] SetDisplayMode ExclusiveFS: FindClosestMatchingMode failed");
                    bFullScreenTransition.store(false);
                    threadManager.threadVars.bSettingFullScreen.store(false);
                    return false;
                }
                ...
                #if defined(_DEBUG_RENDERER_)
                    debug.logDebugMessage(LogLevel::LOG_DEBUG,
                        L"[RENDERER] SetDisplayMode ExclusiveFS: closest mode %dx%d @%dHz",
                        closestMode.Width, closestMode.Height,
                ...
                // Release all swap-chain-dependent resources before any mode change.
                ...
                // Tells DXGI the intended output resolution before exclusive ownership is taken;
                // avoids the black-screen that can result when SetFullscreenState picks a mode
                // that the monitor does not natively support.
                ...
                // Step 3 — ResizeTarget again with refresh rate zeroed per Microsoft guidance.
                // Prevents DXGI from attempting a second unsolicited mode switch after exclusive
                // fullscreen is entered, which can cause an additional black-screen flash.
                ...
                if (FAILED(hr))
                {
                    debug.logLevelMessage(LogLevel::LOG_ERROR, L"[RENDERER] SetDisplayMode ExclusiveFS: ResizeBuffers failed");
                    m_swapChain->SetFullscreenState(FALSE, nullptr);
                    threadManager.threadVars.bIsResizing.store(false);
                    bFullScreenTransition.store(false);
                    threadManager.threadVars.bSettingFullScreen.store(false);
                ...
                if (FAILED(hr))
                {
                    debug.logLevelMessage(LogLevel::LOG_ERROR, L"[RENDERER] SetDisplayMode ExclusiveFS: GetBuffer failed");
                    m_swapChain->SetFullscreenState(FALSE, nullptr);
                    threadManager.threadVars.bIsResizing.store(false);
                    bFullScreenTransition.store(false);
                    threadManager.threadVars.bSettingFullScreen.store(false);
                ...
                if (FAILED(hr))
                {
                    debug.logLevelMessage(LogLevel::LOG_ERROR, L"[RENDERER] SetDisplayMode ExclusiveFS: CreateRenderTargetView failed");
                    m_swapChain->SetFullscreenState(FALSE, nullptr);
                    threadManager.threadVars.bIsResizing.store(false);
                    bFullScreenTransition.store(false);
                    threadManager.threadVars.bSettingFullScreen.store(false);
                ...
                // Sync tracked dimensions from the confirmed back buffer.
                {
                ...
                m_isExclusiveFullscreen = true;
                iOrigWidth  = static_cast<UINT>(m_renderTargetWidth);
                iOrigHeight = static_cast<UINT>(m_renderTargetHeight);
                ...
                winMetrics.isBorderless    = false;
                winMetrics.monitorFullArea = { 0, 0, static_cast<LONG>(iOrigWidth), static_cast<LONG>(iOrigHeight) };
                winMetrics.monitorWorkArea = { 0, 0, static_cast<LONG>(iOrigWidth), static_cast<LONG>(iOrigHeight) };
                ...
                debug.logLevelMessage(LogLevel::LOG_INFO, L"[RENDERER] SetDisplayMode: exclusive fullscreen engaged");
                return true;
            }
            catch (const std::exception& e)
            {
                debug.logLevelMessage(LogLevel::LOG_CRITICAL,
                    L"[RENDERER] SetDisplayMode ExclusiveFS exception: " +
                    std::wstring(e.what(), e.what() + strlen(e.what())));
                threadManager.threadVars.bIsResizing.store(false);
                bFullScreenTransition.store(false);
                threadManager.threadVars.bSettingFullScreen.store(false);
        ...
        default:
            debug.logLevelMessage(LogLevel::LOG_WARNING, L"[RENDERER] SetDisplayMode: unknown mode — ignoring");
            return false;
    }
}
