            ...
            m_3dTexSetLayout != VK_NULL_HANDLE && m_descriptorPool != VK_NULL_HANDLE)
        {
            ...
            if (vkAllocateDescriptorSets(m_device, &dsai, &m_defaultTexSetDescSet) == VK_SUCCESS)
            {
                VkImageView fallbackViews[4] = {
                    m_defaultTexture.view,   // diffuse
                    m_defaultNormalTex.view, // normal
                    m_defaultOrmTex.view,    // ORM
                    m_defaultAoTex.view      // AO
                };
...
void VulkanRenderer::CreateDescriptorSetLayouts()
{
    ...
    std::array<VkDescriptorSetLayoutBinding, 4> tex3DBindings{};
    for (uint32_t b = 0; b < 4; ++b) {
...
void main() {
    vec4 albedo = texture(diffuseTex, vTexCoord);
    albedo.rgb *= mat.Kd;
...
{
    // Per-model: 4 samplers (diffuse/normal/ORM/AO) + 2 UBOs (transform + material).
    ...
    sizes[0] = { VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 4096 }; // 512 models × 4 textures + 2D headroom
    sizes[1] = { VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,         1024 }; // 512 models × 2 UBOs
                                         ...
                                         const float FontSize, float controlWidth, float controlHeight, bool /*bold*/)
{
    ...
    // Full-rect centering: pass the entire control rect and let DWrite centre both axes via
    // PARAGRAPH_ALIGNMENT_CENTER + TEXT_ALIGNMENT_CENTER — more accurate than manual metric math.
