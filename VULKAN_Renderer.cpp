...
VulkanRenderer::VulkanRenderer()
{
    ...
    for (auto& fd : m_frames) {
        ...
        fd.renderFinished = VK_NULL_HANDLE;
...
void VulkanRenderer::Cleanup()
{
    ...
    for (auto& fd : m_frames) {
        ...
        if (fd.renderFinished != VK_NULL_HANDLE) vkDestroySemaphore(m_device, fd.renderFinished, nullptr);
...
{
    // Texture binding (binding 0 = combined image sampler)
...
// Inline GLSL source for the 3D lit geometry pipeline
...
layout(location = 0) out vec4 outColor;
void main() {
    vec4  albedo  = texture(diffuseTex, fragUV);
    vec3  N       = normalize(fragNormal);
    vec3  L       = normalize(-lpc.lightDir);
    float diff    = max(dot(N, L), 0.0);
    vec3  V       = normalize(lpc.viewPos - fragWorldPos);
    vec3  H       = normalize(L + V);
    float spec    = pow(max(dot(N, H), 0.0), 32.0);
    vec3  ambient = lpc.ambientStrength * lpc.lightColor;
    vec3  light   = ambient + (diff + spec * 0.5) * lpc.intensity * lpc.lightColor;
...
void VulkanRenderer::CreateGraphicsPipelines()
{
    ...
    if (vert3D != VK_NULL_HANDLE && frag3D != VK_NULL_HANDLE) {
        ...
        rast3D.cullMode    = VK_CULL_MODE_BACK_BIT;
        ...
        pc3D.offset     = 0;
        pc3D.size       = sizeof(float) * 13; // lightDir+intensity+lightColor+ambient+viewPos

        std::array<VkDescriptorSetLayout, 2> layouts3D = { m_textureDescSetLayout, m_uniformDescSetLayout };
...
void VulkanRenderer::CreateDescriptorPool()
{
    ...
    sizes[0] = { VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 512 };
    sizes[1] = { VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,         64  };
...
void VulkanRenderer::CreateSyncObjects()
{
    ...
    for (auto& fd : m_frames) {
        ...
        vkCreateSemaphore(m_device, &sci, nullptr, &fd.renderFinished);
...
void VulkanRenderer::CreateOverlayResources(uint32_t width, uint32_t height)
{
    ...
    m_wicFactory->CreateBitmap(width, height, GUID_WICPixelFormat32bppPBGRA,
                                WICBitmapCacheOnLoad, &m_wicBitmap);

    // D2D factory
    D2D1CreateFactory(D2D1_FACTORY_TYPE_SINGLE_THREADED, m_d2dFactory.GetAddressOf());

    // D2D render target from WIC bitmap
    D2D1_RENDER_TARGET_PROPERTIES rtp = D2D1::RenderTargetProperties(
        D2D1_RENDER_TARGET_TYPE_DEFAULT,
        D2D1::PixelFormat(DXGI_FORMAT_B8G8R8A8_UNORM, D2D1_ALPHA_MODE_PREMULTIPLIED));
    m_d2dFactory->CreateWicBitmapRenderTarget(m_wicBitmap.Get(), rtp, &m_d2dRenderTarget);
    ...
    // Vulkan upload texture + staging buffer for the overlay
    m_overlayTexture = CreateTextureFromRGBA(nullptr, width, height);
...
{
    bool ok = threadManager.StartThread(THREAD_RENDERER, [this]() { RenderFrame(); });
    ok &= threadManager.StartThread(THREAD_LOADER,   [this]() { LoaderTaskThread(); });
    return ok;
}
...
bool VulkanRenderer::SetFullScreen()
{
    ...
    SetWindowPos(m_hwnd, HWND_TOP, mi.rcMonitor.left, mi.rcMonitor.top, w, h, SWP_FRAMECHANGED);
...
bool VulkanRenderer::SetWindowedScreen()
{
    ...
    SetWindowPos(m_hwnd, nullptr,
        ...
        DEFAULT_WINDOW_WIDTH, DEFAULT_WINDOW_HEIGHT, SWP_FRAMECHANGED | SWP_NOZORDER);
    IsWindowMode.store(true);
    return Resize(DEFAULT_WINDOW_WIDTH, DEFAULT_WINDOW_HEIGHT);
#elif defined(PLATFORM_LINUX) || defined(PLATFORM_ANDROID)
    return true;
#endif
}
...
#if defined(PLATFORM_WINDOWS)
    // Use WIC to decode the image on Windows
...
VkImageView VulkanRenderer::CreateImageView(VkImage image, VkFormat format, VkAspectFlags aspect) const
{
...
void VulkanRenderer::DrawRectangleD2D(const Vector2& pos, const Vector2& size, const MyColor& color)
{
    ...
    m_d2dRenderTarget->FillRectangle(rect, brush.Get());
}
...
void VulkanRenderer::UploadOverlayToVulkan(VkCommandBuffer cmd)
{
    ...
    void* mapped = nullptr;
    VkDeviceSize mapSize = static_cast<VkDeviceSize>(m_overlayWidth) * m_overlayHeight * 4;
    ...
    for (uint32_t row = 0; row < m_overlayHeight; ++row)
        std::memcpy(static_cast<uint8_t*>(mapped) + row * m_overlayWidth * 4,
                    dataPtr + row * stride, m_overlayWidth * 4);
...
{
    return GetCharacterWidth(character, FontSize, FontName);
}

float VulkanRenderer::GetCharacterWidth(wchar_t character, float FontSize, const std::wstring& fontName)
{
#if defined(PLATFORM_WINDOWS)
    if (!m_dwriteFactory) return FontSize * 0.6f;
    IDWriteTextFormat* fmt = GetOrCreateTextFormat(fontName.c_str(), FontSize);
    if (!fmt) return FontSize * 0.6f;
    std::wstring ch(1, character);
    ComPtr<IDWriteTextLayout> layout;
    if (FAILED(m_dwriteFactory->CreateTextLayout(ch.c_str(), 1, fmt, 1000.0f, 1000.0f, &layout))) return FontSize * 0.6f;
...
{
#if defined(PLATFORM_WINDOWS)
    if (!m_dwriteFactory) return static_cast<float>(text.size()) * FontSize * 0.6f;
    IDWriteTextFormat* fmt = GetOrCreateTextFormat(FontName, FontSize);
    if (!fmt) return static_cast<float>(text.size()) * FontSize * 0.6f;
    ComPtr<IDWriteTextLayout> layout;
    if (FAILED(m_dwriteFactory->CreateTextLayout(text.c_str(), static_cast<UINT32>(text.size()),
                fmt, containerWidth, 1000.0f, &layout))) return 0.0f;
    DWRITE_TEXT_METRICS metrics{};
    layout->GetMetrics(&metrics);
    return metrics.widthIncludingTrailingWhitespace;
#else
...
{
#if defined(PLATFORM_WINDOWS)
    if (!m_dwriteFactory) return FontSize;
    IDWriteTextFormat* fmt = GetOrCreateTextFormat(FontName, FontSize);
    if (!fmt) return FontSize;
    ComPtr<IDWriteTextLayout> layout;
    if (FAILED(m_dwriteFactory->CreateTextLayout(text.c_str(), static_cast<UINT32>(text.size()),
                fmt, static_cast<float>(m_renderTargetWidth), containerHeight, &layout))) return FontSize;
    DWRITE_TEXT_METRICS metrics{};
    layout->GetMetrics(&metrics);
    return metrics.height;
#else
    (void)text; (void)containerHeight;
    return FontSize;
#endif
}
                                  ...
                                  const MyColor& tintColor, bool is2D)
{
    ...
    if (is2D && textureId >= 0 && textureId < MAX_TEXTURE_BUFFERS && m_textures2D[textureId].isValid) {
        // Draw using the 2D Vulkan texture — blit via overlay approach
        Blit2DObjectToSize(static_cast<BlitObj2DIndexType>(textureId),
...
void VulkanRenderer::Blit2DObject(BlitObj2DIndexType iIndex, int iX, int iY)
{
    ...
    if (idx < 0 || idx >= MAX_TEXTURE_BUFFERS || !m_textures2D[idx].isValid) return;
    // Will render overlay via Vulkan quad after D2D done
    // Mark dirty so overlay is re-uploaded this frame
    m_overlayDirty = true;
    (void)iX; (void)iY;
#elif defined(PLATFORM_LINUX) || defined(PLATFORM_ANDROID)
...
void VulkanRenderer::Blit2DObjectToSize(BlitObj2DIndexType iIndex, int iX, int iY, int iWidth, int iHeight)
{
    ...
    if (idx < 0 || idx >= MAX_TEXTURE_BUFFERS || !m_textures2D[idx].isValid) return;
    m_overlayDirty = true;
    (void)iX; (void)iY; (void)iWidth; (void)iHeight;
#elif defined(PLATFORM_LINUX) || defined(PLATFORM_ANDROID)
...
// ---------------------------------------------------------------------------------------------------------------
// RenderBackgroundImage (mirrors DX11 approach — draws BG image to 2D overlay)
// ---------------------------------------------------------------------------------------------------------------
void VulkanRenderer::RenderBackgroundImage()
{
    ...
    int bgIdx = static_cast<int>(BlitObj2DIndexType::BG_INTRO);
    if (bgIdx >= 0 && bgIdx < MAX_TEXTURE_BUFFERS && m_textures2D[bgIdx].isValid)
        m_overlayDirty = true;
#elif defined(PLATFORM_LINUX) || defined(PLATFORM_ANDROID)
    // Background clear handled by Vulkan render pass clear color
#endif
}
