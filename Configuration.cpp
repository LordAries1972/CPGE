...
bool Configuration::loadConfig() {
    ...
    // Validate renderer against the compiled backend AFTER checksum check so the
    // raw stored value is used for checksum verification, not the adjusted one.
    {
        int corrected = ValidateRendererForPlatform(myConfig.rendererType);
        if (corrected != myConfig.rendererType) {
            debug.logLevelMessage(LogLevel::LOG_WARNING,
                L"[Configuration] rendererType in config does not match compiled backend — overriding and saving.");
            myConfig.rendererType = corrected;
            saveConfig();
        } else {
            myConfig.rendererType = corrected;
        }
...
int Configuration::ValidateRendererForPlatform(int type)
{
    ...
    // Fall back to whatever renderer was compiled into this binary.
#if defined(__USE_OPENGL__)
    return 2;
