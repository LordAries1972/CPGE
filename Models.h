...
struct Material {
    ...
    // --- UV settings (parsed from the importer) ---
    // FBX texture UV transform (translation/scaling/rotation) is baked into the
    // vertex UVs at import; wrap modes feed ModelInfo::uvWrapU/V for the
    // per-renderer diffuse sampler (0=REPEAT 1=CLAMP 2=MIRROR).
    float uvTranslationU = 0.0f;
    float uvTranslationV = 0.0f;
    float uvScalingU     = 1.0f;
    float uvScalingV     = 1.0f;
...
struct ModelInfo {
    ...
    // --- Geometry pre-cache system ---
    // sourceSceneFile: the GLB/GLTF that produced this models[] entry.
    // bGpuReady:       GPU buffers and scene-level instance data are valid;
    //                  fast-path reloads can skip file I/O and GPU setup.
    // cachedInstanceIndex: the scene_models[] slot used when this was last instanced.
    std::wstring sourceSceneFile;
    bool bGpuReady           = false;
    int  cachedInstanceIndex = -1;
    ...
    //   0 = REPEAT (default), 1 = CLAMP_TO_EDGE, 2 = MIRRORED_REPEAT
    int uvWrapU = 0;
    int uvWrapV = 0;
