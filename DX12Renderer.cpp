...
void DX12Renderer::DrawCurve(float startX, float startY, float ctrlX, float ctrlY, float endX, float endY,
                              const MyColor& color, float thickness, bool is2D) {
    if (!is2D || !IsDX11CompatibilityAvailable() || !m_d2dContext || !m_d2dFactory) return;
    ...
    m_d2dContext->DrawGeometry(pathGeometry.Get(), brush.Get(), thickness);
}
...
float DX12Renderer::GetCharacterWidth(wchar_t character, float FontSize) {
    ...
    try {
        ...
        // Return the width of the character. widthIncludingTrailingWhitespace, not width —
        // DWRITE_TEXT_METRICS::width excludes trailing whitespace, and since this layout
        // holds exactly one character, a lone space is always "trailing" and would
        // otherwise measure as ~0, throwing off cursor placement for every space typed.
        float charWidth = textMetrics.widthIncludingTrailingWhitespace;
...
float DX12Renderer::GetCharacterWidth(wchar_t character, float FontSize, const std::wstring& fontName) {
    ...
    try {
        ...
        // Return the width of the character using the specified font (see trailing-whitespace note above)
        float charWidth = textMetrics.widthIncludingTrailingWhitespace;
...
float DX12Renderer::GetCharacterWidth(wchar_t character, float FontSize, bool bold) {
    ...
    try {
        ...
        return textMetrics.widthIncludingTrailingWhitespace; // see trailing-whitespace note above
    }
    catch (...) { return 0.0f; }
}
...
//-----------------------------------------
// Set Borderless Fullscreen Mode for DirectX 12
// Uses MonitorFromWindow + WS_POPUP — swap chain stays windowed, no SetFullscreenState(TRUE).
// This is the Microsoft-recommended approach on Windows 10/11 to avoid DWM deadlocks.
//-----------------------------------------
bool DX12Renderer::SetFullScreen(void)
{
#if defined(_DEBUG_DX12RENDERER_) && defined(_DEBUG)
    debug.logLevelMessage(LogLevel::LOG_INFO, L"DX12Renderer: SetFullScreen() called - beginning borderless fullscreen transition");
#endif
    ...
    try {
        ...
        // Save current windowed size for potential return to windowed mode
        RECT rc;
        ...
        // Use MonitorFromWindow so multi-monitor setups and spanning windows are handled correctly.
        // GetContainingOutput is NOT used — on Windows 10/11 it can trigger a message-queue
        // deadlock with DWM when called inside s_renderMutex.
        // bSettingFullScreen is already true so WM_SIZE generated by SetWindowPos is suppressed
        // inside WindowProc before it can re-enter the renderer.
        HMONITOR hMonitor = MonitorFromWindow(hwnd, MONITOR_DEFAULTTONEAREST);
        MONITORINFO monInfo = {};
        monInfo.cbSize = sizeof(MONITORINFO);
        if (!GetMonitorInfo(hMonitor, &monInfo)) {
            debug.logLevelMessage(LogLevel::LOG_ERROR, L"DX12Renderer: SetFullScreen: GetMonitorInfo failed");
            bFullScreenTransition.store(false);
            threadManager.threadVars.bSettingFullScreen.store(false);
            return false;
        }
        UINT fullscreenWidth  = static_cast<UINT>(monInfo.rcMonitor.right  - monInfo.rcMonitor.left);
        UINT fullscreenHeight = static_cast<UINT>(monInfo.rcMonitor.bottom - monInfo.rcMonitor.top);

#if defined(_DEBUG_DX12RENDERER_) && defined(_DEBUG)
        debug.logDebugMessage(LogLevel::LOG_DEBUG, L"DX12Renderer: Borderless fullscreen target: %dx%d at monitor (%d,%d)",
            fullscreenWidth, fullscreenHeight, monInfo.rcMonitor.left, monInfo.rcMonitor.top);
#endif

        // Strip title bar and resize borders; DXGI swap chain stays windowed.
        // WS_POPUP + SetWindowPos to cover the full monitor is recommended on Windows 10/11.
        SetWindowLong(hwnd, GWL_STYLE, WS_POPUP);
        SetWindowPos(hwnd, HWND_TOP,
                     monInfo.rcMonitor.left, monInfo.rcMonitor.top,
                     static_cast<int>(fullscreenWidth), static_cast<int>(fullscreenHeight),
                     SWP_FRAMECHANGED | SWP_SHOWWINDOW);

        threadManager.threadVars.bIsResizing.store(true);

        // Release D2D resources — must happen before ResizeBuffers
        if (m_d2dContext) {
        ...
        // Release DX12 back-buffer resources held by frame contexts
        for (UINT i = 0; i < FrameCount; ++i)
            m_frameContexts[i].renderTarget.Reset();
        m_depthStencilBuffer.Reset();
        ...
        // Resize swap chain to the monitor resolution — swap chain stays windowed (no SetFullscreenState)
        HRESULT hr = m_swapChain->ResizeBuffers(
            m_effectiveFrameCount,
            fullscreenWidth,
            fullscreenHeight,
        ...
        if (FAILED(hr)) {
            debug.logLevelMessage(LogLevel::LOG_ERROR, L"DX12Renderer: SetFullScreen: ResizeBuffers failed");
            threadManager.threadVars.bIsResizing.store(false);
            bFullScreenTransition.store(false);
            threadManager.threadVars.bSettingFullScreen.store(false);
            return false;
        ...
        // Recreate render target views for all active back buffers
        CD3DX12_CPU_DESCRIPTOR_HANDLE rtvHandle(m_rtvHeap.cpuStart);
        for (UINT i = 0; i < m_effectiveFrameCount; ++i) {
            hr = m_swapChain->GetBuffer(i, IID_PPV_ARGS(&m_frameContexts[i].renderTarget));
            if (FAILED(hr)) {
                debug.logDebugMessage(LogLevel::LOG_ERROR, L"DX12Renderer: SetFullScreen: GetBuffer[%u] failed", i);
                threadManager.threadVars.bIsResizing.store(false);
                bFullScreenTransition.store(false);
                threadManager.threadVars.bSettingFullScreen.store(false);
                return false;
        ...
        // Update internal dimensions BEFORE CreateDepthStencilBuffer (it reads iOrigWidth/iOrigHeight)
        iOrigWidth           = static_cast<int>(fullscreenWidth);
        iOrigHeight          = static_cast<int>(fullscreenHeight);
        m_renderTargetWidth  = static_cast<int>(fullscreenWidth);
        m_renderTargetHeight = static_cast<int>(fullscreenHeight);
        m_frameIndex = m_swapChain->GetCurrentBackBufferIndex();

        CreateDepthStencilBuffer();

        // Recreate D2D render targets wrapping the new back buffers
        if (m_d2dContext && m_dx11Dx12Compat.dx11On12Device) {
            if (!CreateD2DRenderTargets())
                debug.logLevelMessage(LogLevel::LOG_WARNING, L"DX12Renderer: SetFullScreen: Failed to recreate D2D render targets");
        }

        // Update window metrics — borderless is NOT exclusive fullscreen, swap chain stays windowed
        ...
        // Update camera so 3D projection and screen-space calculations use the real display size
        {
        ...
        debug.logLevelMessage(LogLevel::LOG_INFO, L"DX12Renderer: Borderless fullscreen set successfully");
        return true;
    }
    catch (const std::exception& e) {
        std::wstring errorMsg = std::wstring(e.what(), e.what() + strlen(e.what()));
...
//-----------------------------------------
// Set Exclusive Fullscreen Mode for DirectX 12
// Follows the Microsoft-recommended 4-step DXGI sequence:
//   1. FindClosestMatchingMode  2. ResizeTarget  3. SetFullscreenState(TRUE)
//   4. ResizeTarget(zero refresh)  5. ResizeBuffers  6. Recreate resources
//-----------------------------------------
bool DX12Renderer::SetFullExclusive(uint32_t width, uint32_t height)
{
    ...
    // Capture desktop mode BEFORE any display change so Cleanup() can restore it
    ...
    // Switch monitor resolution BEFORE acquiring s_renderMutex.
    // ChangeDisplaySettingsEx pumps the Windows message queue internally; if called while
    // holding the mutex, DXGI intercepts WM_DISPLAYCHANGE and tries to acquire its internal
    // swap-chain lock — then SetFullscreenState (inside the mutex) needs that same lock,
    // producing a cross-lock deadlock.  Running the mode switch here lets DXGI finish all
    // display-change processing before we take the mutex.
    {
        DEVMODE dm    = {};
    ...
    try {
        ...
        // Obtain the DXGI output (monitor). Fall back to adapter output 0 if the swap chain
        // is not yet associated with a display (early in startup).
        ComPtr<IDXGIOutput> output;
        HRESULT hr = m_swapChain->GetContainingOutput(&output);
        if (FAILED(hr) || !output) {
            debug.logLevelMessage(LogLevel::LOG_WARNING, L"DX12Renderer: SetFullExclusive: GetContainingOutput failed, trying adapter fallback");
            ComPtr<IDXGIDevice> dxgiDevice;
            ...
            if (!output) {
                debug.logLevelMessage(LogLevel::LOG_WARNING, L"DX12Renderer: SetFullExclusive: No display output found — exclusive fullscreen skipped");
                bFullScreenTransition.store(false);
                threadManager.threadVars.bSettingFullScreen.store(false);
                return false;
            }
        ...
        // Find the closest supported display mode — prevents black-screen on unsupported resolutions
        ...
        hr = output->FindClosestMatchingMode(&targetMode, &closestMode, nullptr);
        if (FAILED(hr)) {
            debug.logLevelMessage(LogLevel::LOG_ERROR, L"DX12Renderer: SetFullExclusive: FindClosestMatchingMode failed");
            bFullScreenTransition.store(false);
            threadManager.threadVars.bSettingFullScreen.store(false);
            return false;
        }
        ...
        debug.logDebugMessage(LogLevel::LOG_DEBUG, L"DX12Renderer: Closest matching mode: %dx%d @%dHz",
            ...
            closestMode.RefreshRate.Numerator / std::max(1u, closestMode.RefreshRate.Denominator));
#endif

        threadManager.threadVars.bIsResizing.store(true);

        // Release all swap-chain-dependent D2D and DX12 resources before any mode change
        if (m_d2dContext) {
            m_d2dContext->SetTarget(nullptr);
            m_d2dContext->Flush();
            D2DBusy.store(false);
        ...
        // Step 1 — Hint DXGI about the intended output resolution before taking exclusive ownership.
        // Avoids the black-screen that results when SetFullscreenState picks an unsupported mode.
        hr = m_swapChain->ResizeTarget(&closestMode);
        if (FAILED(hr))
            debug.logLevelMessage(LogLevel::LOG_WARNING, L"DX12Renderer: SetFullExclusive: ResizeTarget (step 1) failed — continuing");

        // Step 2 — Take exclusive ownership of the output
        hr = m_swapChain->SetFullscreenState(TRUE, output.Get());
        if (FAILED(hr)) {
            debug.logLevelMessage(LogLevel::LOG_ERROR, L"DX12Renderer: SetFullExclusive: SetFullscreenState(TRUE) failed");
            threadManager.threadVars.bIsResizing.store(false);
            bFullScreenTransition.store(false);
            threadManager.threadVars.bSettingFullScreen.store(false);
            return false;
        ...
        // Step 3 — Zero refresh rate prevents DXGI from making a second unsolicited mode switch
        DXGI_MODE_DESC zeroRefMode = closestMode;
        zeroRefMode.RefreshRate.Numerator   = 0;
        zeroRefMode.RefreshRate.Denominator = 0;
        m_swapChain->ResizeTarget(&zeroRefMode);

        // Step 4 — Resize back buffers to the confirmed exclusive resolution
        ...
        if (FAILED(hr)) {
            debug.logLevelMessage(LogLevel::LOG_ERROR, L"DX12Renderer: SetFullExclusive: ResizeBuffers failed");
            m_swapChain->SetFullscreenState(FALSE, nullptr);
            threadManager.threadVars.bIsResizing.store(false);
            bFullScreenTransition.store(false);
            threadManager.threadVars.bSettingFullScreen.store(false);
        ...
        for (UINT i = 0; i < m_effectiveFrameCount; ++i) {
            hr = m_swapChain->GetBuffer(i, IID_PPV_ARGS(&m_frameContexts[i].renderTarget));
            if (FAILED(hr)) {
                debug.logDebugMessage(LogLevel::LOG_ERROR, L"DX12Renderer: SetFullExclusive: GetBuffer[%u] failed", i);
                m_swapChain->SetFullscreenState(FALSE, nullptr);
                threadManager.threadVars.bIsResizing.store(false);
                bFullScreenTransition.store(false);
                threadManager.threadVars.bSettingFullScreen.store(false);
        ...
        // Update internal dimensions BEFORE CreateDepthStencilBuffer (it reads iOrigWidth/iOrigHeight)
        iOrigWidth           = static_cast<int>(closestMode.Width);
        iOrigHeight          = static_cast<int>(closestMode.Height);
        m_renderTargetWidth  = static_cast<int>(closestMode.Width);
        m_renderTargetHeight = static_cast<int>(closestMode.Height);
        m_frameIndex = m_swapChain->GetCurrentBackBufferIndex();
        ...
        if (m_d2dContext && m_dx11Dx12Compat.dx11On12Device) {
            if (!CreateD2DRenderTargets())
                debug.logLevelMessage(LogLevel::LOG_WARNING, L"DX12Renderer: SetFullExclusive: Failed to recreate D2D render targets");
        }

        m_isExclusiveFullscreen = true;

        // Sync winMetrics so every subsystem sees a coherent coordinate space
        winMetrics.x               = 0;
        winMetrics.y               = 0;
        ...
        // Update camera so 3D projection uses the actual exclusive fullscreen resolution
        {
            float ar = LookupAspectRatio(m_renderTargetWidth, m_renderTargetHeight);
            config.myConfig.aspectRatio = ar;
            myCamera.UpdateResolution(iOrigWidth, iOrigHeight, ar);
        }
...
#if defined(_DEBUG_DX12RENDERER_) && defined(_DEBUG)
        debug.logDebugMessage(LogLevel::LOG_INFO, L"DX12Renderer: Exclusive fullscreen set successfully at %dx%d", closestMode.Width, closestMode.Height);
        {
...
bool DX12Renderer::SetWindowedScreen(void)
{
    ...
    try {
        ...
        // Exit exclusive fullscreen state if we are in it
        if (m_swapChain) {
            HRESULT hr = m_swapChain->SetFullscreenState(FALSE, nullptr);
            if (FAILED(hr)) {
                debug.logLevelMessage(LogLevel::LOG_ERROR, L"DX12Renderer: SetWindowedScreen: Failed to exit fullscreen state");
                bFullScreenTransition.store(false);
                threadManager.threadVars.bSettingFullScreen.store(false);
                return false;
            }
        ...
        // Early exit during shutdown — fullscreen state is all we needed to release
        ...
        // Restore the windowed window style that SetFullScreen replaced with WS_POPUP
        SetWindowLong(hwnd, GWL_STYLE, WS_OVERLAPPEDWINDOW);

        // Resolve windowed dimensions
        ...
        // Release D2D resources before ResizeBuffers
        if (m_d2dContext) {
            m_d2dContext->SetTarget(nullptr);
            m_d2dContext->Flush();
            D2DBusy.store(false);
        ...
        // Release DX12 back-buffer and depth resources
        for (UINT i = 0; i < FrameCount; ++i)
            m_frameContexts[i].renderTarget.Reset();
        m_depthStencilBuffer.Reset();
        ...
        if (FAILED(hr)) {
            debug.logLevelMessage(LogLevel::LOG_ERROR, L"DX12Renderer: SetWindowedScreen: ResizeBuffers failed");
            threadManager.threadVars.bIsResizing.store(false);
            bFullScreenTransition.store(false);
            threadManager.threadVars.bSettingFullScreen.store(false);
            return false;
        ...
        for (UINT i = 0; i < m_effectiveFrameCount; ++i) {
            hr = m_swapChain->GetBuffer(i, IID_PPV_ARGS(&m_frameContexts[i].renderTarget));
            if (FAILED(hr)) {
                debug.logDebugMessage(LogLevel::LOG_ERROR, L"DX12Renderer: SetWindowedScreen: GetBuffer[%u] failed", i);
                threadManager.threadVars.bIsResizing.store(false);
                bFullScreenTransition.store(false);
                threadManager.threadVars.bSettingFullScreen.store(false);
                return false;
        ...
        // Update internal dimensions BEFORE CreateDepthStencilBuffer (it reads iOrigWidth/iOrigHeight)
        iOrigWidth           = static_cast<int>(windowedWidth);
        iOrigHeight          = static_cast<int>(windowedHeight);
        m_renderTargetWidth  = static_cast<int>(windowedWidth);
        m_renderTargetHeight = static_cast<int>(windowedHeight);
        m_frameIndex = m_swapChain->GetCurrentBackBufferIndex();
        ...
        if (m_d2dContext && m_dx11Dx12Compat.dx11On12Device) {
            if (!CreateD2DRenderTargets())
                debug.logLevelMessage(LogLevel::LOG_WARNING, L"DX12Renderer: SetWindowedScreen: Failed to recreate D2D render targets");
        }

        // Update winMetrics
        winMetrics.isFullScreen   = false;
        winMetrics.isBorderless   = false;
        winMetrics.width          = windowedWidth;
        winMetrics.height         = windowedHeight;
        winMetrics.clientWidth    = windowedWidth;
        winMetrics.clientHeight   = windowedHeight;

        // Update camera
        {
            float newAR = LookupAspectRatio(static_cast<int>(windowedWidth), static_cast<int>(windowedHeight));
            config.myConfig.aspectRatio = newAR;
            myCamera.UpdateResolution(windowedWidth, windowedHeight, newAR);
        }
        ...
        int centerX = (workArea.right  - workArea.left - static_cast<int>(windowedWidth))  / 2;
        int centerY = (workArea.bottom - workArea.top  - static_cast<int>(windowedHeight)) / 2;
        SetWindowPos(hwnd, nullptr, centerX, centerY,
            static_cast<int>(windowedWidth), static_cast<int>(windowedHeight),
...
// -------------------------------------------------------------------------------------------------
// SetDisplayMode — unified display-mode entry point following Microsoft/DXGI guidelines:
//
//   Windowed             : restore windowed window and swap chain (delegates to SetWindowedScreen)
//   BorderlessFullscreen : WS_POPUP + SetWindowPos to full monitor, swap chain stays windowed
//                          (delegates to SetFullScreen — recommended default on Windows 10/11)
//   ExclusiveFullscreen  : DXGI guide-compliant sequence:
//                            FindClosestMatchingMode → ResizeTarget → SetFullscreenState(TRUE)
//                            → ResizeTarget (zeroed refresh) → ResizeBuffers → recreate resources
//
// The no-arg overload reads displayMode/resolutionWidth/resolutionHeight/refreshRate from config.
// -------------------------------------------------------------------------------------------------
bool DX12Renderer::SetDisplayMode()
{
...
bool DX12Renderer::SetDisplayMode(DisplayMode mode, int width, int height, int refreshHz)
{
    switch (mode)
    {
        case DisplayMode::Windowed:
        {
            bool r = SetWindowedScreen();
            if (r) guiManager.OnWindowResize(iOrigWidth, iOrigHeight);
            return r;
        }

        case DisplayMode::BorderlessFullscreen:
        {
            bool r = SetFullScreen();
            if (r) guiManager.OnWindowResize(iOrigWidth, iOrigHeight);
            return r;
        }
        ...
        {
            if (width <= 0 || height <= 0) {
                debug.logLevelMessage(LogLevel::LOG_ERROR, L"DX12Renderer: SetDisplayMode: invalid exclusive fullscreen dimensions");
                return false;
            }
            if (bFullScreenTransition.load()) {
                debug.logLevelMessage(LogLevel::LOG_WARNING, L"DX12Renderer: SetDisplayMode: fullscreen transition already in progress");
                return false;
            }
            ...
            // Capture desktop mode for restore on shutdown before any display change occurs
            m_originalDesktopMode = {};
            m_originalDesktopMode.dmSize = sizeof(DEVMODE);
            EnumDisplaySettings(nullptr, ENUM_CURRENT_SETTINGS, &m_originalDesktopMode);
            ...
            try
            {
                ...
                // Obtain the DXGI output. Fall back to adapter output 0 when the swap chain
                // is not yet associated with an output (early in startup).
                ComPtr<IDXGIOutput> output;
                HRESULT hr = m_swapChain->GetContainingOutput(&output);
                if (FAILED(hr) || !output) {
                    debug.logLevelMessage(LogLevel::LOG_WARNING,
                        L"DX12Renderer: SetDisplayMode ExclusiveFS: GetContainingOutput failed — falling back to adapter output 0");
                    ComPtr<IDXGIDevice> dxgiDev;
                    if (SUCCEEDED(m_dx11Dx12Compat.dx11Device.As(&dxgiDev))) {
                        ComPtr<IDXGIAdapter> dxgiAdapter;
                        if (SUCCEEDED(dxgiDev->GetAdapter(&dxgiAdapter)))
                            dxgiAdapter->EnumOutputs(0, &output);
                    }
                    if (!output) {
                        debug.logLevelMessage(LogLevel::LOG_ERROR, L"DX12Renderer: SetDisplayMode ExclusiveFS: cannot obtain IDXGIOutput");
                        bFullScreenTransition.store(false);
                        threadManager.threadVars.bSettingFullScreen.store(false);
                        return false;
                    }
                ...
                // Find the closest supported display mode — prevents black-screen on unsupported resolutions
                DXGI_MODE_DESC targetMode = {};
                targetMode.Width  = static_cast<UINT>(width);
                targetMode.Height = static_cast<UINT>(height);
                targetMode.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
                targetMode.RefreshRate.Numerator   = static_cast<UINT>(refreshHz > 0 ? refreshHz : config.myConfig.refreshRate);
                targetMode.RefreshRate.Denominator = 1;
                targetMode.ScanlineOrdering = DXGI_MODE_SCANLINE_ORDER_UNSPECIFIED;
                targetMode.Scaling          = DXGI_MODE_SCALING_UNSPECIFIED;
                ...
                hr = output->FindClosestMatchingMode(&targetMode, &closestMode, nullptr);
                if (FAILED(hr)) {
                    debug.logLevelMessage(LogLevel::LOG_ERROR, L"DX12Renderer: SetDisplayMode ExclusiveFS: FindClosestMatchingMode failed");
                    bFullScreenTransition.store(false);
                    threadManager.threadVars.bSettingFullScreen.store(false);
                    return false;
                }
...
#if defined(_DEBUG_DX12RENDERER_) && defined(_DEBUG)
                debug.logDebugMessage(LogLevel::LOG_DEBUG,
                    L"DX12Renderer: SetDisplayMode ExclusiveFS: closest mode %dx%d @%dHz",
                    closestMode.Width, closestMode.Height,
                    closestMode.RefreshRate.Numerator / std::max(1u, closestMode.RefreshRate.Denominator));
#endif
                ...
                // Release all swap-chain-dependent resources
                if (m_d2dContext) { m_d2dContext->SetTarget(nullptr); m_d2dContext->Flush(); D2DBusy.store(false); }
                for (UINT i = 0; i < FrameCount; ++i) {
                    m_d2dRenderTargets[i].Reset();
                    m_wrappedBackBuffers[i].Reset();
                }
                ...
                // Step 1 — ResizeTarget with the confirmed closest mode
                hr = m_swapChain->ResizeTarget(&closestMode);
                if (FAILED(hr))
                    debug.logLevelMessage(LogLevel::LOG_WARNING, L"DX12Renderer: SetDisplayMode ExclusiveFS: ResizeTarget (step 1) failed — continuing");

                // Step 2 — Take exclusive ownership of the output
                hr = m_swapChain->SetFullscreenState(TRUE, output.Get());
                if (FAILED(hr)) {
                    debug.logLevelMessage(LogLevel::LOG_ERROR, L"DX12Renderer: SetDisplayMode ExclusiveFS: SetFullscreenState(TRUE) failed");
                    threadManager.threadVars.bIsResizing.store(false);
                    bFullScreenTransition.store(false);
                    threadManager.threadVars.bSettingFullScreen.store(false);
                    return false;
                ...
                // Step 3 — Zero refresh rate to prevent a second unsolicited mode switch
                DXGI_MODE_DESC zeroRefMode = closestMode;
                zeroRefMode.RefreshRate.Numerator   = 0;
                zeroRefMode.RefreshRate.Denominator = 0;
                m_swapChain->ResizeTarget(&zeroRefMode);
                ...
                if (FAILED(hr)) {
                    debug.logLevelMessage(LogLevel::LOG_ERROR, L"DX12Renderer: SetDisplayMode ExclusiveFS: ResizeBuffers failed");
                    m_swapChain->SetFullscreenState(FALSE, nullptr);
                    threadManager.threadVars.bIsResizing.store(false);
                    bFullScreenTransition.store(false);
                    threadManager.threadVars.bSettingFullScreen.store(false);
                ...
                for (UINT i = 0; i < m_effectiveFrameCount; ++i) {
                    hr = m_swapChain->GetBuffer(i, IID_PPV_ARGS(&m_frameContexts[i].renderTarget));
                    if (FAILED(hr)) {
                        debug.logDebugMessage(LogLevel::LOG_ERROR, L"DX12Renderer: SetDisplayMode ExclusiveFS: GetBuffer[%u] failed", i);
                        m_swapChain->SetFullscreenState(FALSE, nullptr);
                        threadManager.threadVars.bIsResizing.store(false);
                        bFullScreenTransition.store(false);
                        threadManager.threadVars.bSettingFullScreen.store(false);
                ...
                // Update dimensions BEFORE CreateDepthStencilBuffer (it reads iOrigWidth/iOrigHeight)
                iOrigWidth           = static_cast<int>(closestMode.Width);
                iOrigHeight          = static_cast<int>(closestMode.Height);
                m_renderTargetWidth  = static_cast<int>(closestMode.Width);
                m_renderTargetHeight = static_cast<int>(closestMode.Height);
                ...
                // Recreate D2D render targets
                if (m_d2dContext && m_dx11Dx12Compat.dx11On12Device) {
                    if (!CreateD2DRenderTargets())
                        debug.logLevelMessage(LogLevel::LOG_WARNING, L"DX12Renderer: SetDisplayMode ExclusiveFS: Failed to recreate D2D render targets");
                }
                ...
                debug.logLevelMessage(LogLevel::LOG_INFO, L"DX12Renderer: SetDisplayMode: exclusive fullscreen engaged");
                return true;
            }
            catch (const std::exception& e) {
                std::wstring errorMsg = std::wstring(e.what(), e.what() + strlen(e.what()));
                debug.logDebugMessage(LogLevel::LOG_CRITICAL,
                    L"DX12Renderer: SetDisplayMode ExclusiveFS exception: %s", errorMsg.c_str());
                threadManager.threadVars.bIsResizing.store(false);
                bFullScreenTransition.store(false);
                threadManager.threadVars.bSettingFullScreen.store(false);
                return false;
        ...
        default:
            debug.logLevelMessage(LogLevel::LOG_WARNING, L"DX12Renderer: SetDisplayMode: unknown mode — ignoring");
            return false;
    }
}
