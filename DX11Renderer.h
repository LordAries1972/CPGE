...
public:
    ...
    ComPtr<ID2D1SolidColorBrush> m_pixelBrush{ nullptr };                       // Reusable brush for Blit2DColoredPixel
    ...
    // CPU-path video frame render — reads from a pre-decoded BGRA CPU buffer instead of mapping
    // the D3D11 staging texture. Use this inside D2D BeginDraw to avoid SEH exception 0x87D
    // (D3D11 debug layer: context->Map() forbidden between BeginDraw and EndDraw).
    void DrawVideoFrameFromCPU(const Vector2& position, const Vector2& size, const MyColor& tintColor,
        const BYTE* pPixels, UINT width, UINT height, UINT rowPitch);
    ...
    bool SetDisplayMode() override;  // Reads mode/width/height/refreshHz from config
    ...
    void DrawMyTextStyled(const std::wstring& text, const Vector2& position,
        const MyColor& color, const TextRenderStyle& style) override;
...
private:
    ...
    // GPU capability flags — populated during swap-chain creation, used for adaptive quality
    ...
    struct TextFormatKey {
        ...
        bool operator==(const TextFormatKey& o) const noexcept {
            return fontSize == o.fontSize && fontName == o.fontName && weight == o.weight;
        }
    };
    struct TextFormatKeyHash {
        size_t operator()(const TextFormatKey& k) const noexcept {
            ...
            h ^= std::hash<uint32_t>()(static_cast<uint32_t>(k.weight)) + 0x9e3779b9u + (h << 6) + (h >> 2);
            return h;
        }
    };
