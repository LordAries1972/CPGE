...
// -----------------------------------
        ...
        // ------------------------------------------------------------------------------------
        // --- Renderer Selection (Windows) ---
        // ------------------------------------------------------------------------------------
        // NOTE: if using CMAKE to build, there is no need to uncomment these lines — 
        // CMAKE will define the appropriate __USE_*__ macro based on the selected renderer 
        ...
        // ------------------------------------------------------------------------------------
            ...
            // Fall back to the bare Windows SDK gl.h if GLEW is not installed.
                ...
                #ifndef GLEW_STATIC
                #define GLEW_STATIC         // Required for glew32s.lib; prevents __declspec(dllimport) on all GLEW symbols
                #endif
                #pragma warning(push)
            ...
            // your shaders for better performance on Radeon hardware.
            #include <RadeonGPUAnalyzer/RadeonGPUAnalyzer.h>
            #pragma comment(lib, "RadeonGPUAnalyzer.lib")
        #endif
...
#else
    #define PLATFORM_NAME_W L"Unknown"
#endif

#if defined(__USE_DIRECTX_11__)
    #define RENDERER_NAME_W L"DirectX 11"
#elif defined(__USE_DIRECTX_12__)
    #define RENDERER_NAME_W L"DirectX 12"
#elif defined(__USE_OPENGL__)
...
// Version overlay build-type prefix: "Debug" in _DEBUG builds, "Release" otherwise.
#if defined(_DEBUG)
    #define BUILD_TYPE_W L"Debug"
#else
    #define BUILD_TYPE_W L"Release"
#endif
...
// ── Assembly optimisation routines (Windows x64 only) ───────────────────────
// MemoryCopy / MemoryZero / MatrixCopy4x4F — ml64.exe MASM routines.
// These replace memcpy / memset in all render-frame hot paths for
// hardware-bandwidth throughput (REP MOVSQ / REP STOSQ / SSE MOVUPS).
// Header is included unconditionally on _WIN64; non-Windows builds
// retain their CRT implementations via standard memcpy / memset.
#if defined(_WIN64)
    #include "assembly.h"
#endif
...
// Models geometry cache — written on clean exit, loaded on startup to skip full GLTF re-parse.
#define MODELS_CACHE_FILENAME "cache.dat"

// ------------------------------------------------------------------------------------
// Call-Stack Log  (ExceptionHandler.cpp — _DEBUG builds only)
// ------------------------------------------------------------------------------------
// Written immediately after any unhandled exception or fatal signal in _DEBUG builds.
// The log contains a human-readable header (exception type, code, fault address, PID,
// TID, platform) followed by up to 25 annotated stack frames traced back from the
// crash point, and a breadcrumb trail of the last RECORD_FUNCTION_CALL() entries.
//
// How to use:
//   - No manual call needed for crash logging; ExceptionHandler writes it automatically.
//   - Sprinkle  RECORD_FUNCTION_CALL()  in high-level functions you want tracked in
//     the breadcrumb trail (already a no-op in Release builds).
//   - After a crash, open the file in any text editor — no special tools required.
//
// Where the file is written (platform-specific):
//   Windows  : process working directory  (next to the .exe)
//   Linux    : process working directory
//   macOS    : process working directory
//   Android  : /data/local/tmp/
//   iOS      : $TMPDIR  (the app sandbox temp folder, guaranteed writable)
//
// Only produced in _DEBUG builds; Release builds skip writing entirely.
// ------------------------------------------------------------------------------------
#define CALLSTACK_LOG_FILENAME "Except-CallStack.log"
...
const int SLOT_SHADOW_SAMPLER_STATE = 2;                                        // Shadow Comparison Sampler (PCF)
    ...
    // Loader text vertical position as a fraction of screen height (all pipelines)
    constexpr float LOADER_TEXT_Y_RATIO = 0.65f;
