...
void GUIManager::CreateGameMenuWindow(const std::wstring& message) {
    ...
    // Quit button: fade in the quit confirmation dialog instead of shutting
    // down immediately. The dialog's OK button performs the actual fade-to-black
    // shutdown sequence; CANCEL returns to the game menu.
    quitButton.onMouseBtnDown = [this]()
    {
        try {
            debug.logDebugMessage(LogLevel::LOG_INFO, L"CreateGameMenuWindow - Quit button clicked, showing quit confirmation dialog");
            soundManager.PlayImmediateSFX(SFX_ID::SFX_BEEP);
...
void GUIManager::CreateQuitConfirmDialog()
{
    ...
    // Chrome (semi-transparent body + magenta/purple gradient title bar) comes from the
    // QuitWindow template — see GUITemplates.h/.cpp. Only content controls are added below.
    CreateMyWindow(
        ...
        GUIWindowTemplateType::QuitWindow,
        L"CONFIRM EXIT"
    );
    ...
    // Safe to show now — fade in from transparent so the dialog materialises.
    dlgWin->isVisible = true;
    ApplyWindowFade(GUIWindowFadeType::FadeIn, 0.35f, QUIT_CONFIRM_WINDOW_NAME);
}
