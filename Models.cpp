...
Texture::~Texture() {
    if (bTextureDestroyed) return;
            ...
            if (renderer) {
                auto vkr = std::dynamic_pointer_cast<VulkanRenderer>(renderer);
                if (vkr && vkr->GetVkDevice() != VK_NULL_HANDLE) {
                    VkDevice dev = vkr->GetVkDevice();
                    if (imageView != VK_NULL_HANDLE) { vkDestroyImageView(dev, imageView, nullptr); imageView = VK_NULL_HANDLE; }
...
bool Texture::LoadFromFile(const std::wstring& path)
{
    ...
    texturePath = path;
        ...
        // Allocate a tight RGBA buffer and copy pixels out of WIC.
    ...
    #elif defined(__USE_VULKAN__)
    {
        auto vkr = std::dynamic_pointer_cast<VulkanRenderer>(renderer);
        if (!vkr) {
            debug.logLevelMessage(LogLevel::LOG_ERROR, L"[Texture] Vulkan: renderer unavailable");
            return false;
        }
        VkDevice         dev   = vkr->GetVkDevice();
        VkPhysicalDevice phys  = vkr->GetVkPhysicalDevice();
        VkCommandPool    pool  = vkr->GetLoaderCommandPool();
        VkQueue          queue = vkr->GetGraphicsQueue();
        if (dev == VK_NULL_HANDLE || pool == VK_NULL_HANDLE || queue == VK_NULL_HANDLE)
            return false;

        std::mutex& queueMtx = vkr->GetQueueMutex();

        // Release any previous Vulkan texture resources before replacing them
        if (imageView != VK_NULL_HANDLE) {
            vkDestroyImageView(dev, imageView, nullptr); imageView = VK_NULL_HANDLE;
            vkDestroyImage(dev, image, nullptr);         image     = VK_NULL_HANDLE;
    ...
    {
        // Delete any previous OpenGL texture before replacing.
        if (textureID != 0) {
            glDeleteTextures(1, &textureID);
            textureID = 0;
        }
        // Delegate to the stb_image-based utility in OpenGLModels.cpp.
        textureID = OpenGLModelUtils::LoadGLTextureFromFile(path);
        return textureID != 0;
    }
    #else
        return false;
...
#elif defined(__USE_OPENGL__)
{
    ...
    // Delegate to the stb_image-based utility in OpenGLModels.cpp.
    textureID = OpenGLModelUtils::LoadGLTextureFromMemory(data, size);
    return textureID != 0;
}
#else
    return false;
...
#elif defined(__USE_VULKAN__)
{
    ...
    Vector4 col{ color.x, color.y, color.z, color.w };
    imageView = VulkanModelUtils::CreateSolidColourTexture(dev, phys, pool, queue,
                                                            col, image, memory,
                                                            &queueMtx, width, height);
    return imageView != VK_NULL_HANDLE;
}
#elif defined(__USE_OPENGL__)
{
    ...
    // color is Vector4 on OpenGL builds (XMFLOAT4 aliased via Includes.h).
    Vector4 col{ color.x, color.y, color.z, color.w };
    textureID = OpenGLModelUtils::CreateSolidColourTexture(col);
    return textureID != 0;
}
#else
    return false;
...
void Model::DestroyModel()
{
...
#if defined(__USE_VULKAN__)
    {
        ...
        {
            // Wait for all GPU work to complete before freeing any Vulkan objects.
            // Use WaitForGPUToFinish() (holds m_queueMutex) rather than calling
            // vkDeviceWaitIdle directly — the render thread holds that mutex during
            // submit/present, and calling vkDeviceWaitIdle from a second thread while
            // the render thread holds the queue triggers a Vulkan threading error.
            vkr->WaitForGPUToFinish();

            if (m_modelInfo.uniformBufferMapped) {
                vkUnmapMemory(device, m_modelInfo.uniformBufferMemory);
                m_modelInfo.uniformBufferMapped = nullptr;
            }
            if (m_modelInfo.uniformBuffer != VK_NULL_HANDLE) {
                vkDestroyBuffer(device, m_modelInfo.uniformBuffer, nullptr);
                vkFreeMemory(device, m_modelInfo.uniformBufferMemory, nullptr);
            ...
            if (m_modelInfo.vertexBuffer != VK_NULL_HANDLE) {
                ...
                m_modelInfo.vertexBuffer        = VK_NULL_HANDLE;
                m_modelInfo.vertexBufferMemory  = VK_NULL_HANDLE;
            }
            if (m_modelInfo.indexBuffer != VK_NULL_HANDLE) {
                vkDestroyBuffer(device, m_modelInfo.indexBuffer, nullptr);
                vkFreeMemory(device, m_modelInfo.indexBufferMemory, nullptr);
    ...
    // =========================================================
    // Release OpenGL GPU Resources
    // =========================================================
#if defined(__USE_OPENGL__)
    if (m_modelInfo.EBO) { glDeleteBuffers(1,      &m_modelInfo.EBO); m_modelInfo.EBO = 0; }
    if (m_modelInfo.VBO) { glDeleteBuffers(1,      &m_modelInfo.VBO); m_modelInfo.VBO = 0; }
    if (m_modelInfo.VAO) { glDeleteVertexArrays(1, &m_modelInfo.VAO); m_modelInfo.VAO = 0; }
    for (GLuint& t : m_modelInfo.textureIDs)
        if (t) { glDeleteTextures(1, &t); t = 0; }
    for (GLuint& t : m_modelInfo.normalMapIDs)
        if (t) { glDeleteTextures(1, &t); t = 0; }
    m_modelInfo.textureIDs.clear();
    m_modelInfo.normalMapIDs.clear();
    // Shader program is owned by the renderer — just null our reference
    m_modelInfo.shaderProgram = 0;
    m_modelInfo.bGpuReady = false;
#endif
...
void Model::ApplyDefaultLightingFromManager(LightsManager& myLightsManager)
{
    ...
    // DX11 uploads all global lights to cbuffer b3 (SLOT_GLOBAL_LIGHT_BUFFER) in RenderGamePlay().
    // The shader accumulates b1 and b3 into the same directLighting variable, so filling
    // localLights (b1) with the same global lights causes every light to be processed twice,
    // making the scene 2x too bright. Leave b1 empty for DX11; b3 covers all lights.
    return;
#endif
...
bool Model::LoadOBJ(const std::string& path)
{
    ...
    for (size_t i=0; i<m_modelInfo.vertices.size(); ++i) {
        ...
        // tangent[3] = handedness sign used by GLSL: B = cross(N,T) * tangent.w
        // Default to +1 — computed tangents from UV layout have standard winding.
        m_modelInfo.vertices[i].tangent[3]=1.0f;
    }
#endif
...
void Model::LoadFallbackNormalMap()
{
    ...
    #if defined(__USE_DIRECTX_12__)
        // UploadDX12Textures() resolves texture slots via findTex(), which searches
        // m_modelInfo.textures by raw SRV pointer.  Without this entry, the flat
        // normal's SRV has no matching Texture object and slot t1 receives a null
        // GPU descriptor, causing undefined normal sampling on all untextured models.
        bool alreadyPresent = false;
        for (auto& t : m_modelInfo.textures)
            if (t.get() == flatNormal.get()) { alreadyPresent = true; break; }
        if (!alreadyPresent)
            m_modelInfo.textures.push_back(flatNormal);
    #endif
#endif
}
...
bool Model::SetupModelForRendering()
{
    ...
    // Lock texture updates while we potentially apply fallbacks.
    {
        ...
        // Ensure t1 always has a valid SRV from the start.  Matches DX12 11on12
        // SetupModelForRendering() which also calls LoadFallbackNormalMap() here.
        // A null t1 on some drivers samples (0,0,0,0) regardless of NormalScale,
        // collapsing all lighting to black.
...
lightDesc.Usage = D3D11_USAGE_DYNAMIC;
    ...
    //=== SAMPLER ===//
    // UV settings: honour the importer's wrap modes (GLTF sampler wrapS/wrapT,
    // FBX WrapModeU/V) parsed into ModelInfo: 0=REPEAT 1=CLAMP 2=MIRROR.
    auto dxWrap = [](int wrapMode) -> D3D11_TEXTURE_ADDRESS_MODE {
        if (wrapMode == 1) return D3D11_TEXTURE_ADDRESS_CLAMP;
        if (wrapMode == 2) return D3D11_TEXTURE_ADDRESS_MIRROR;
        return D3D11_TEXTURE_ADDRESS_WRAP;
    };
    D3D11_SAMPLER_DESC sampDesc = {};
    sampDesc.Filter = D3D11_FILTER_MIN_MAG_MIP_LINEAR;
    sampDesc.AddressU = dxWrap(m_modelInfo.uvWrapU);
    sampDesc.AddressV = dxWrap(m_modelInfo.uvWrapV);
...
#elif defined(__USE_VULKAN__)
{
    if (!renderer) {
        debug.logLevelMessage(LogLevel::LOG_WARNING,
            L"[Model] SetupModelForRendering (Vulkan): renderer is null for model ID " + std::to_wstring(m_modelInfo.ID));
        return false;
    }
    ...
    if (!vkr) {
        debug.logLevelMessage(LogLevel::LOG_WARNING,
            L"[Model] SetupModelForRendering (Vulkan): renderer cast failed for model ID " + std::to_wstring(m_modelInfo.ID));
        return false;
    }

    VkDevice         device    = vkr->GetVkDevice();
    VkPhysicalDevice physDev   = vkr->GetVkPhysicalDevice();
    VkCommandPool    cmdPool   = vkr->GetLoaderCommandPool();
    VkQueue          queue     = vkr->GetGraphicsQueue();
    std::mutex&      queueMtx  = vkr->GetQueueMutex();

    if (device == VK_NULL_HANDLE || physDev == VK_NULL_HANDLE ||
        cmdPool == VK_NULL_HANDLE || queue  == VK_NULL_HANDLE) {
        debug.logLevelMessage(LogLevel::LOG_WARNING,
            L"[Model] SetupModelForRendering (Vulkan): device/pool/queue not ready for model ID " + std::to_wstring(m_modelInfo.ID));
        return false;
    }
    ...
    if (!vkVerts.empty()) {
        VkDeviceSize vbSize = vkVerts.size() * sizeof(VkVertex3D);
        if (!VulkanModelUtils::UploadVertexBuffer(device, physDev, cmdPool, queue,
                vkVerts.data(), vbSize,
                m_modelInfo.vertexBuffer, m_modelInfo.vertexBufferMemory, &queueMtx)) {
            debug.logLevelMessage(LogLevel::LOG_ERROR,
                L"[Model] SetupModelForRendering (Vulkan): vertex buffer upload failed, model ID " + std::to_wstring(m_modelInfo.ID));
            return false;
        }
    }

    if (!m_modelInfo.indices.empty()) {
        if (!VulkanModelUtils::UploadIndexBuffer(device, physDev, cmdPool, queue,
                m_modelInfo.indices.data(), m_modelInfo.indices.size(),
                m_modelInfo.indexBuffer, m_modelInfo.indexBufferMemory, &queueMtx)) {
            debug.logLevelMessage(LogLevel::LOG_ERROR,
                L"[Model] SetupModelForRendering (Vulkan): index buffer upload failed, model ID " + std::to_wstring(m_modelInfo.ID));
            return false;
        }
    }

    // The pipeline is owned by the renderer, NOT the model.
    // Storing the renderer's pipeline handle here would cause vkDestroyPipeline
    // to be called on the shared pipeline when the model is cleaned up.
    // RenderGamePlay accesses m_3dPipeline directly from the renderer — no need
    // to duplicate the handle in ModelInfo.
    m_modelInfo.pipeline       = VK_NULL_HANDLE;
    m_modelInfo.pipelineLayout = VK_NULL_HANDLE;
    ...
    if (tex3DLayout != VK_NULL_HANDLE && m_modelInfo.textureDescriptorSet == VK_NULL_HANDLE) {
        ...
        if (vkAllocateDescriptorSets(device, &dsai, &m_modelInfo.textureDescriptorSet) == VK_SUCCESS) {
            ...
            // Gloss/emissive fall back to the white diffuse texture; shader skips sampling when use*Map==0.
            VkImageView fallbackViews[6] = {
                vkr->GetDefaultDiffuseView(), // 0: diffuse  -- 1x1 white
                vkr->GetDefaultNormalView(),  // 1: normal   -- 1x1 flat normal
                vkr->GetDefaultOrmView(),     // 2: ORM      -- no metallic, mid roughness
                vkr->GetDefaultAoView(),      // 3: AO       -- 1x1 white
                vkr->GetDefaultDiffuseView(), // 4: gloss    -- white fallback (useGlossMap=0 disables sampling)
                vkr->GetDefaultDiffuseView()  // 5: emissive -- white fallback (useEmissiveMap=0 disables sampling)
            };
            std::array<VkWriteDescriptorSet, 6> texWrites{};
            std::array<VkDescriptorImageInfo,  6> texImgInfos{};
            ...
            vkUpdateDescriptorSets(device, 6, texWrites.data(), 0, nullptr);
        }
    }

    #if defined(_DEBUG_SCENEMANAGER_) || defined(_DEBUG_MODEL_)
        debug.logLevelMessage(LogLevel::LOG_DEBUG,
            L"[Model] SetupModelForRendering (Vulkan): GPU buffers ready for model ID " + std::to_wstring(m_modelInfo.ID));
    #endif
    return true;
}
...
#elif defined(__USE_OPENGL__)
{
    if (m_modelInfo.vertices.empty()) {
        debug.logLevelMessage(LogLevel::LOG_WARNING,
            L"[Model] SetupModelForRendering (OpenGL): no vertices for model ID " + std::to_wstring(m_modelInfo.ID));
        return false;
    }

    // Destroy any previous GPU allocation
    if (m_modelInfo.VAO) { glDeleteVertexArrays(1, &m_modelInfo.VAO); m_modelInfo.VAO = 0; }
    if (m_modelInfo.VBO) { glDeleteBuffers(1,      &m_modelInfo.VBO); m_modelInfo.VBO = 0; }
    if (m_modelInfo.EBO) { glDeleteBuffers(1,      &m_modelInfo.EBO); m_modelInfo.EBO = 0; }

    // Upload vertex + index data via OpenGLModelBuffers
    OpenGLModelBuffers buf;
    if (!buf.Upload(m_modelInfo.vertices.data(),
                    m_modelInfo.vertices.size() * sizeof(Vertex),
                    m_modelInfo.indices.data(),
                    m_modelInfo.indices.size()))
    {
        debug.logLevelMessage(LogLevel::LOG_ERROR,
            L"[Model] SetupModelForRendering (OpenGL): GPU buffer upload failed for model ID " + std::to_wstring(m_modelInfo.ID));
        return false;
    }
    m_modelInfo.VAO = buf.VAO;
    m_modelInfo.VBO = buf.VBO;
    m_modelInfo.EBO = buf.EBO;
    // Detach from buf so it doesn't destroy them in its destructor
    buf.VAO = buf.VBO = buf.EBO = 0;

    // Assign the renderer's built-in 3D shader program (per-model override not needed by default)
    if (renderer && m_modelInfo.shaderProgram == 0) {
        if (auto* r = dynamic_cast<OpenGLRenderer*>(renderer.get()))
            m_modelInfo.shaderProgram = r->GetBuiltin3DShaderID();
    }

    // Build the GL texture-handle lists from the model's material set.
    // Shared with the SceneManager cache-restore rebind path (Step 4), which
    // re-parses materials after Setup has already run and must refresh the IDs.
    RefreshOpenGLTextures();

    m_modelInfo.bGpuReady = true;

    #if defined(_DEBUG_SCENEMANAGER_) || defined(_DEBUG_MODEL_)
        debug.logLevelMessage(LogLevel::LOG_DEBUG,
            L"[Model] SetupModelForRendering (OpenGL): GPU buffers ready for model ID " + std::to_wstring(m_modelInfo.ID));
    #endif
    return true;
}
#else
    // Non-DX11/DX12/Vulkan/OpenGL: no GPU setup required
    return true;
#endif
}
...
void Model::Render(ID3D11DeviceContext* deviceContext, float deltaTime)
{
    ...
    // Guard: all three GPU buffers must exist before any D3D11 calls are made.
    // Matches the DX12 11on12 Render() guard — prevents binding null resources
    // if SetupModelForRendering has not yet completed for this model slot.
    if (!m_modelInfo.vertexBuffer || !m_modelInfo.indexBuffer || !m_modelInfo.constantBuffer)
        return;

    // Bind shaders first so the 11on12 compat context is ready before
    // UpdateConstantBuffer calls Map/Unmap.  Matches DX12 11on12 Render() ordering.
    ...
    // Updates the constant buffer with the current world matrix (b0).
    ...
    // Bind b0 (world/view/proj/camera/scale) to both VS and PS
    ...
    if (m_modelInfo.materialBuffer)
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            // Unmap and immediately bind to PS b4 — matches DX12 11on12 Render() ordering
            // where PSSetConstantBuffers is inside the SUCCEEDED block so b4 is only
            // bound when the buffer actually contains fresh data this frame.
    ...
    // LoadFallbackNormalMap() is now called once in SetupModelForRendering() so
    // normalMapSRVs[0] is always valid; no per-frame LoadFallbackNormalMap needed.
...
// ============================================================================
// Model::Render (Vulkan / OpenGL)
//
// Draw calls for these renderers are issued inline from the renderer's
// RenderGamePlay loop (binding VBOs / VkBuffers per-model, then calling
// glDrawElements / vkCmdDrawIndexed directly).  This method provides the
// per-frame CPU-side update hook (e.g. animation state) and is the required
// definition of the declared symbol — without it the linker produces an error.
// ============================================================================
void Model::Render(float deltaTime)
{
    if (!m_isLoaded || bIsDestroyed)                                      return;
    ...
    // Accumulate per-model animation time (used by GLTF keyframe sampling).
    m_animationTime += deltaTime;

    // fxActive is reset each frame by the renderer before calling Render/RenderGamePlay.
    // Nothing else to do here; all GPU commands are issued by VulkanRenderer::RenderGamePlay
    // or the equivalent OpenGL render loop.
}

#endif // __USE_DIRECTX_11__ || __USE_DIRECTX_12__ || __USE_OPENGL__ || __USE_VULKAN__
