...
void Model::DestroyModel()
{
...
#if defined(__USE_VULKAN__)
    {
        ...
        {
            // The render thread may have submitted command buffers that reference this
            // model's vertex/index/uniform buffers but the GPU hasn't finished executing
            // them yet.  Wait for all GPU work to complete before freeing any Vulkan
            // objects — this is a scene-transition path so the stall is acceptable.
...
#elif defined(__USE_OPENGL__)
{
    ...
    // Load diffuse textures from the model's material list
    for (auto& [matName, mat] : m_materials) {
        ...
        } else if (!mat.diffuseMapPath.empty()) {
        ...
        } else if (!mat.normalMapPath.empty()) {
    ...
    // Fallback: create a 1x1 white texture if no diffuse was loaded
