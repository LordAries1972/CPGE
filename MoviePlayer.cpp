...
bool MoviePlayer::OpenMovie(const std::wstring& filePath)
{
    ...
    // Hold m_mutex for the entire teardown so that UpdateFrame() (which acquires
    // the same mutex via try_to_lock) cannot concurrently dereference m_pCurrentSample,
    // m_pSourceReader, or m_pSourceVoice while we free them here.  The render thread's
    // try_to_lock will fail and return false (skip this frame) while we hold the lock.
    // Lock ordering: m_mutex → DX11 GetRenderMutex (consistent with UpdateVideoTexture).
    {
        std::lock_guard<std::recursive_mutex> teardownLock(m_mutex);

        // Release all per-file resources without shutting down Media Foundation or the
        // XAudio2 engine.  This avoids the expensive MFShutdown()+MFStartup() round-trip
        // that Reset() performs, so back-to-back movie opens are near-instantaneous.
        m_isPlaying.store(false);
        m_isPaused.store(false);
        m_playStartMediaTime = -1;
        m_hasNewFrame.store(false);

        // Destroy the per-file source voice; format may differ for the new file.
        // The XAudio2 engine and mastering voice are reused across movies.
        ...
        // Release MF per-file objects
        if (m_pCurrentSample)  { m_pCurrentSample->Release();  m_pCurrentSample  = nullptr; }
        if (m_pVideoMediaType) { m_pVideoMediaType->Release(); m_pVideoMediaType = nullptr; }
        if (m_pSourceReader)   { m_pSourceReader->Release();   m_pSourceReader   = nullptr; }

        // Release renderer-side texture objects.
        ...
        m_hasVideo.store(false);
        m_hasAudio.store(false);
        m_videoWidth          = 0;
        m_videoHeight         = 0;
        m_videoDuration       = 0;
        m_audioReadPosition   = 0;
...
bool MoviePlayer::UpdateVideoTexture()
{
    ...
    // Unmap BEFORE CopyResource — D3D11 spec forbids CopyResource on a mapped subresource;
    // the SDK validation layer raises an SEH exception (0x87D) if this order is violated.
    context->Unmap(destTexture, 0);

    // Unlock the media buffer before the GPU copy so MF can reclaim the buffer.
    buffer->Unlock();

    // If we have a dual-texture system, push the decoded frame into the bindable render texture.
...
// ----------------------------------------------------------------------------------------------
// Reset the MoviePlayer: tear down all resources, reset every internal variable, and
// reinitialize Media Foundation (and audio) so the next OpenMovie() call starts clean.
//
// Platform guards:
//   Windows  - Media Foundation / XAudio2 full shutdown + reinit
//   Linux    - GStreamer path (stub, implementation pending)
//   macOS    - AVFoundation path (stub, implementation pending)
// ----------------------------------------------------------------------------------------------
bool MoviePlayer::Reset()
{
#if defined(PLATFORM_WINDOWS)

    debug.logLevelMessage(LogLevel::LOG_INFO, L"MoviePlayer: Resetting...");

    // Acquire the recursive mutex before tearing down state so that no concurrent
    // UpdateFrame() or Render() call races against the release sequence.
    std::lock_guard<std::recursive_mutex> lock(m_mutex);

    // Full teardown: releases all MF COM objects, DirectX textures, audio voices, and
    // calls MFShutdown().  m_renderer, m_threadManager, m_enableAudio, and m_targetFPS
    // are intentionally left untouched so the player can accept OpenMovie() immediately.
    Cleanup();

    // Reset variables not zeroed by Cleanup()
    m_llCurrentPosition.store(0);
    m_audioReadPosition   = 0;
    m_firstAudioSamplePTS = -1;
    m_audioStartSamples   = 0;
    ...
    // Reinitialize Media Foundation (matches the MFShutdown called inside Cleanup())
    if (!InitializeMF())
    {
        debug.logLevelMessage(LogLevel::LOG_ERROR, L"MoviePlayer: Reset - Failed to reinitialize Media Foundation");
        return false;
    }

    // Recreate the XAudio2 engine and mastering voice if audio was enabled during Initialize()
    if (m_enableAudio)
    {
        if (!InitializeAudio())
        {
            debug.logLevelMessage(LogLevel::LOG_WARNING, L"MoviePlayer: Reset - Audio reinitialization failed, continuing without audio");
            m_enableAudio = false;
        }
    }

    m_isInitialized = true;
    debug.logLevelMessage(LogLevel::LOG_INFO, L"MoviePlayer: Reset complete - ready for next video");
    return true;

#elif defined(PLATFORM_LINUX)

    // Linux: GStreamer-based MoviePlayer reset
    // TODO: call gst_element_set_state(pipeline, GST_STATE_NULL) and reinitialize pipeline
    debug.logLevelMessage(LogLevel::LOG_INFO, L"MoviePlayer::Reset() - Linux/GStreamer path not yet implemented");
    return false;

#elif defined(PLATFORM_MACOS)

    // macOS: AVFoundation-based MoviePlayer reset
    // TODO: [player replaceCurrentItemWithPlayerItem:nil] and rebuild AVPlayer
    debug.logLevelMessage(LogLevel::LOG_INFO, L"MoviePlayer::Reset() - macOS/AVFoundation path not yet implemented");
    return false;

#else
    debug.logLevelMessage(LogLevel::LOG_ERROR, L"MoviePlayer::Reset() - Unsupported platform");
    return false;
#endif
}
...
bool MoviePlayer::UpdateFrame()
{
    ...
    if (m_enableAudio && m_pSourceVoice)
    {
        ...
        // 1000 ms look-ahead limit expressed in MF 100-ns units (increased from 500 ms to prevent crackling at high framerates)
        static constexpr LONGLONG kMaxAudioAheadMF = 10000000LL;
