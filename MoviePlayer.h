...
public:
    ...
    // Vulkan/OpenGL path: returns a CPU-side BGRA frame buffer decoded from the current
    // video sample.  Returns nullptr if no frame is ready or CPU decoding is unavailable.
    const uint8_t* GetCurrentFrameRGBA(uint32_t& outWidth, uint32_t& outHeight);
...
private:
    ...
    // CPU-side frame decode for Vulkan/OpenGL renderers (no D3D11 texture required)
    bool UpdateVideoTextureCPU();
...
private:
    ...
    // CPU-side frame buffer for Vulkan/OpenGL renderers
    std::vector<uint8_t> m_cpuFrameBuffer;
    uint32_t             m_cpuFrameWidth  = 0;
    uint32_t             m_cpuFrameHeight = 0;
