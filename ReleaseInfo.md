...
*Current Build Version: v0.0.1278 — May 22, 2026*
...
- **VS2022 project — renderer-prefixed output EXE** (`CrossPlatformGameEngine.vcxproj`): `PrintSDKDiagnostics` detects the renderer and sets `_RendererPrefix` (`DX` / `DX12` / `OpenGL` / `Vulkan`). A dedicated `SetRendererLinkerOutput` target (`BeforeTargets="Link"`, runs after all `.obj` files are compiled) sets `TargetName`, `TargetFileName`, and `TargetPath` to the prefixed name and also updates `%(Link.OutputFile)` metadata on all compiled `@(Link)` objects via `<ItemGroup Update>`. This gives the linker the correct `/OUT:` path directly — producing `DXCPGE.exe` / `DX12CPGE.exe` / `OpenGLCPGE.exe` / `VulkanCPGE.exe` with no post-link rename — and silences MSB8012 by keeping `TargetName` and `%(Link.Filename)` in sync.
