# PUNPack Compression Class - Comprehensive Usage Guide

## Overview

This document provides comprehensive usage examples for the PUNPack compression class, demonstrating all features including string compression, structure serialization, memory buffer compression, checksum calculation, and performance optimization.
...
### Unicode String Compression Example
...
void ExampleWideStringCompression() {
    ...
    std::vector<std::wstring> testWideStrings = {
        L"Hello, 世界! This is a Unicode test string with émojis 🚀🎮.",
        L"Ññññññññññññññññññññññññññññññññññññññññññññññññññ", // Repetitive Unicode
        L"Русский текст для тестирования сжатия данных with mixed languages.",
        L"数字测试：１２３４５６７８９０１２３４５６７８９０",
        L"Mixed: English 中文 Русский العربية 日本語 한국어"
    };
...
### Structure Serialization Example
...
### Buffer Compression with Different Data Types
...
### Standalone Checksum Calculation Example
...
void ExampleChecksumCalculation() {
    ...
    std::wstring testWString = L"Wide string test data for checksum 测试数据.";
...
- **RLE (Run-Length Encoding)**: Best for data with many repeated values (e.g., bitmaps, simple patterns)
- **LZ77**: Good general-purpose compression for most data types with reasonable performance
- **Huffman**: Excellent for text and data with predictable frequency patterns
- **Hybrid**: Automatically selects the best algorithm based on data characteristics

### 2. Performance Optimization

```cpp
// Use optimal compression type detection for unknown data
CompressionType optimalType = compressor.GetOptimalCompressionType(data, size);
PackResult result = compressor.PackBuffer(data, optimalType, enableEncryption);

// Enable encryption only when necessary (adds overhead)
bool needEncryption = (dataContainsSensitiveInfo);
PackResult result = compressor.PackString(text, CompressionType::LZ77, needEncryption);

// Batch operations for better performance
compressor.ResetStatistics();
// ... perform multiple operations
CompressionStats stats = compressor.GetStatistics();
```

### 3. Error Handling

```cpp
// Always check if PackResult is valid
PackResult packResult = compressor.PackString(data, type, encryption);
if (!packResult.IsValid()) {
...
uint32_t expectedChecksum = compressor.CalculateChecksum(originalData);
if (packResult.checksum != expectedChecksum) {
    // Handle checksum mismatch
    return false;
}

// Check UnpackResult success
UnpackResult unpackResult = compressor.UnpackString(packResult);
if (!unpackResult.success) {
    // Handle decompression failure
    // Check unpackResult.errorMessage for details
    return false;
}
```
...
```cpp
// For large data processing, consider memory usage
const size_t MAX_CHUNK_SIZE = 1024 * 1024; // 1MB chunks

if (dataSize > MAX_CHUNK_SIZE) {
    // Process data in chunks if memory is a concern
    // or use streaming compression if available
}
...
```cpp
// Always verify data integrity after decompression
bool VerifyIntegrity(const std::vector<uint8_t>& original, const std::vector<uint8_t>& decompressed) {
...
// Use checksums for additional verification
uint32_t originalChecksum = compressor.CalculateChecksum(originalData);
// ... compress and decompress
uint32_t decompressedChecksum = compressor.CalculateChecksum(decompressedData);
bool integrityOK = (originalChecksum == decompressedChecksum);
```

### 6. Threading Considerations

```cpp
// PUNPack instances are not thread-safe by default
// Use separate instances for different threads or add synchronization

class ThreadSafePUNPackManager {
private:
    std::vector<std::unique_ptr<PUNPack>> m_compressors;
    std::mutex m_mutex;
...
public:
    bool Initialize(size_t numThreads) {
        ...
        for (size_t i = 0; i < numThreads; ++i) {
            auto compressor = std::make_unique<PUNPack>();
            if (!compressor->Initialize()) {
                return false;
            }
            m_compressors.push_back(std::move(compressor));
        }
    ...
    PUNPack* GetCompressor() {
        std::lock_guard<std::mutex> lock(m_mutex);
        PUNPack* result = m_compressors[m_currentIndex].get();
        m_currentIndex = (m_currentIndex + 1) % m_compressors.size();
        return result;
    }
};
```
...
Make sure to add the debug define to your Debug.h file:

```cpp
// Add this to Debug.h if not already present
#define _DEBUG_PUNPACK_
```

This enables all the debug output in the examples and helps with troubleshooting compression operations.

## Summary

The PUNPack compression class provides a comprehensive solution for data compression needs in game development and other applications. Key features include:

- **Multiple compression algorithms** (RLE, LZ77, Huffman, Hybrid)
- **Automatic optimal algorithm selection**
- **Built-in encryption support**
- **Comprehensive checksum verification**
- **Performance statistics and monitoring**
- **Support for strings, wide strings, structures, and binary data**
- **Robust error handling and data integrity verification**

Use this guide as a reference for implementing PUNPack in your applications, and adapt the examples to fit your specific use cases and requirements.