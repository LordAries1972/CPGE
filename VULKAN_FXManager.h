...
// Per-effect state for ImageFadeStrobe (Vulkan build)
struct VKImageFadeStrobeData {
...
struct VKFXItem {
    ...
    VKImageFadeStrobeData imageFadeStrobeData;                                     // State for the ImageFadeStrobe effect (only used when type == ImageFadeStrobe)
};
...
private:
    ...
    void UpdateImageFadeStrobe(VKFXItem& fx, float deltaTime);                      // Advance strobe alpha each frame (called from Render2D)
